...
This file is the **external vulnerability policy**. Other layers are not copies of it:

| Layer | Where |
| ----- | ----- |
| **Policy (this document)** | [`SECURITY.md`](SECURITY.md) (this file) |
| **Technician guide** | [`docs/SECURITY.md`](docs/SECURITY.md) ([pt-BR](docs/SECURITY.pt_BR.md)) — product tests and operator advice; points **here** for reporting and supported versions |
| **Lab / runtime** | [`docs/ops/DATA_BOAR_LAB_SECURITY_TOOLING.md`](docs/ops/DATA_BOAR_LAB_SECURITY_TOOLING.md) ([pt-BR](docs/ops/DATA_BOAR_LAB_SECURITY_TOOLING.pt_BR.md)) · [`docs/ops/DOCKER_AIDE_AUDITD_BASELINE.md`](docs/ops/DOCKER_AIDE_AUDITD_BASELINE.md) ([pt-BR](docs/ops/DOCKER_AIDE_AUDITD_BASELINE.pt_BR.md)) |
| **Agent enforcement** | [`.cursor/rules/`](.cursor/rules/) (never weaken gates, PII, containment) |
| **Constitution** | ADRs — supply-chain **Layer 1** is [ADR 0074](docs/adr/ADR-0074-supply-chain-layer1-digest-pins-and-rust-sca.md) |
| **Index** | [`docs/SECURITY_GOVERNANCE_POSTURE_HUB.md`](docs/SECURITY_GOVERNANCE_POSTURE_HUB.md) ([pt-BR](docs/SECURITY_GOVERNANCE_POSTURE_HUB.pt_BR.md)) |

### Executor host and detection (L3/L4) — pointer only

**Not shipped in this repo.** Work is tracked in GitHub issue **[#989](https://github.com/DataBoar/data-boar/issues/989)** (OPEN, milestone **v1.8.4**). [ADR 0074](docs/adr/ADR-0074-supply-chain-layer1-digest-pins-and-rust-sca.md) remains **Layer 1** only until that issue lands an ADR amendment.

- **L3 (executor host):** Cursor/Electron (and similar) is a privileged write surface — minimize extensions, prefer OpenVSX / trusted publishers, least privilege for signing keys. Requirements and lab audit live in **#989**, not as extra policy here.
- **L4 (detection):** host tripwires (canary-class files, AIDE + auditd), not a product feature. Lab baseline: [`docs/ops/DOCKER_AIDE_AUDITD_BASELINE.md`](docs/ops/DOCKER_AIDE_AUDITD_BASELINE.md); planting canaries on lab hosts is operator/vault work in **#989**.
...
### Rust crate SCA (`rust/boar_fast_filter`)

Layer 1 supply-chain posture for the Rust prefilter is recorded in [ADR 0074](docs/adr/ADR-0074-supply-chain-layer1-digest-pins-and-rust-sca.md) (Accepted): digest-pinned container bases, Actions SHA pins, and Rust SCA before merge. This section is the **external** policy view (CI + lockfile). Lab/runtime tooling lives under **`docs/ops/DATA_BOAR_LAB_SECURITY_TOOLING.md`** ([pt-BR](docs/ops/DATA_BOAR_LAB_SECURITY_TOOLING.pt_BR.md)).

**Known-advisory scanning** runs in [`.github/workflows/rust-ci.yml`](.github/workflows/rust-ci.yml): **`cargo-audit` 0.22.2** and **`cargo-deny` 0.19.9**, both installed with **`--locked`**. Policy for deny: **`rust/boar_fast_filter/deny.toml`**. Treat **RUSTSEC / GHSA** hits like Dependabot security PRs: **P0 / five business days** to accept a fix, defer with written justification, or document an operator exception — same SLA as Python Dependabot in this file.

**Known-advisory tools do not see a crate that never received an identifier.** Three public crates.io incidents illustrate the gap:

| Incident | When | Identifier | Source |
| -------- | ---- | ---------- | ------ |
| `rustdecimal` (typosquat of `rust_decimal`; payload if `GITLAB_CI` was set) | 2022-05-10 | **RUSTSEC-2022-0042** · **GHSA-7pwq-f4pq-78gm** | [Rust blog](https://blog.rust-lang.org/2022/05/10/malicious-crate-rustdecimal/) |
| `finch-rust` / `sha-rust` | 2025-12-05 | none listed | [Rust blog](https://blog.rust-lang.org/2025/12/05/crates.io-malicious-crates-finch-rust-and-sha-rust) |
| **`arrayref` / `internment` / `append-only-vec`** | 2026-08-20 | **no CVE / RUSTSEC / GHSA assigned** | [Rust blog](https://blog.rust-lang.org/2026/08/20/supply-chain-attack-on-arrayref/) |

On 2026-08-20 a compromised maintainer account published `arrayref@0.3.10`, `internment@0.8.7`, and `append-only-vec@0.1.9` between **07:15 and 07:37 UTC**, each adding a dependency on **`proc-macro1`** (typosquat of **`proc-macro2`**) whose **build script downloaded and ran a remote payload at compile time**. Crates were yanked between 08:41 and 09:25 UTC. **`cargo audit` would not have flagged that window** (86 minutes, no advisory id).

**What actually stops that class here:** committed **`rust/boar_fast_filter/Cargo.lock`** plus **`--locked`** on every CI cargo command. A locked tree does not pick up a freshly published malicious version. In-tree lockfile review: none of those compromised crate versions; **`proc-macro2`** is the legitimate crate (the name `proc-macro1` imitated).
...
| Layer | Pin / lock? | Why |
| ----- | ----------- | --- |
| **`Cargo.lock`** | Yes — committed + `--locked` | Real control against a compromised crate publisher |
| **`cargo-audit` / `cargo-deny`** | Yes — versions pinned in `rust-ci.yml` (0.22.2 / 0.19.9) | A self-updating security tool is its own attack surface |
| **GitHub Actions** | Yes — full commit SHA | Same as ADR 0005 |
| **Base image** | Yes — digest on `Dockerfile` `FROM` | ADR 0074 Layer 1 |
| **`rustc` toolchain** | **No** — do **not** pin in-repo | Rust stability guarantees; pinning delays compiler/std *soundness* fixes and adds fork friction. The three incidents above were **crate** attacks, not toolchain attacks. The `cdylib` + `abi3` surface is a C ABI; a different `rustc` does not by itself produce an incompatible artifact. |

PR **dependency-review** (GitHub `dependency-review-action` in **`ci.yml`**) complements Python/Actions graphs on pull requests. It does **not** replace `Cargo.lock` + `--locked`.
...
| **`sbom/provenance-local.json`** | Unsigned local record: commit, version, toolchain, workflow inputs, SBOM/artifact digests | **`scripts/emit-provenance.sh`** / **`.ps1`** (`scripts/emit_provenance.py`). **Not** SLSA. Tag path verifies against **`data-boar.intoto.jsonl`** |
...
- **Blocked-dependency triage checkpoint (last review: 2026-08-07):** GitHub Dependabot **open** alerts: **none** (historical pyOpenSSL / PyJWT / older pypdf / Sonar action alerts are **fixed**). **`pip-audit`** on this pass required **`pypdf>=6.15.0`** (CVE-2026-71852 / CVE-2026-71870; prior floor **6.14.2**). **`uv.lock`** still carries **`pygments` 2.20.0+** and **`pyopenssl` 26.x**. Re-verify on each **Band A order –1** pass and at least **quarterly** — see **PLANS_TODO.md** *Quarterly blocked-dependency checkpoint*.
- **pyOpenSSL Dependabot alerts (#9 / #10) and Snowflake:** **Resolved** as of **2026-05-22** (connector **4.5.0+**). Historical cap and reopen steps: **[docs/ops/DEPENDABOT_PYOPENSSL_SNOWFLAKE.md](docs/ops/DEPENDABOT_PYOPENSSL_SNOWFLAKE.md)**.
- **Pygments Dependabot / pip-audit (CVE-2026-4539):** **Resolved** as of **2026-05-22** (**pygments 2.20.0** on PyPI). Historical triage and bump steps: **[docs/ops/DEPENDABOT_PYGMENTS_CVE.md](docs/ops/DEPENDABOT_PYGMENTS_CVE.md)**.
...
- **CI workflow supply chain:** Workflows under **`.github/workflows/`** pin third-party GitHub Actions to **full commit SHAs** (version tag in YAML comments for humans). The **`astral-sh/setup-uv`** step pins a **specific uv CLI semver**—not **`latest`**—so installs do not float silently between runs. **Dependabot** may propose SHA bumps; review upstream release notes before merge. **Dockerfile** `FROM` lines are **digest-pinned**; Dependabot’s docker ecosystem proposes digest bumps. Pull requests also run **`dependency-review`**. This **reduces** tag-moving and unexpected action updates but is **not** a guarantee against zero-day compromise of a pinned commit, supply-chain attacks that pass review, or risks outside CI (for example local developer tooling). See **[ADR 0005](docs/adr/ADR-0005-ci-github-actions-supply-chain-pins.md)** and **[ADR 0074](docs/adr/ADR-0074-supply-chain-layer1-digest-pins-and-rust-sca.md)**.
...
- **Path traversal:** `session_id` in API paths is validated with a strict pattern (alphanumeric and underscore, 12–64 chars) before use in file paths or lookups; invalid values return HTTP 400. See **`api/routes.py`** `_validate_session_id` and **`tests/test_security.py`**. Optional **`sql_sampling_file`** / **`sql_sampling_files`** fragments must resolve **inside** the main config directory (absolute paths outside that directory are rejected); YAML/JSON parse errors do not echo file snippets into the exception (so config-save HTTP banners cannot leak foreign file content). See **`config/loader.py`** and **`tests/test_config_sql_sampling_files.py`**.
...
- **Operator notifications (webhooks):** Optional `notifications` can POST scan summaries to Slack, Teams, a generic URL (e.g. Signal bridge), or Telegram fields for legacy/third-party configs. The canonical maintainer policy is **not** to use Telegram for Data Boar—see **`docs/ops/OPERATOR_NOTIFICATION_CHANNELS.md`**. Treat webhook URLs and bot tokens as secrets; use `${ENV_VAR}` in YAML or env-only wiring. See **`docs/USAGE.md`** (§5.1). Sends retry on transient failures (5xx / network); they do not replace TLS or network policy.
- **API bind vs API key:** When starting **`--web`** with a **non-loopback** bind (e.g. `0.0.0.0`), the process **exits 2** unless **`api.require_api_key: true`** and a key is resolved (`core/host_resolution.py` / `#1714`). Presence of `api.api_key` or a WebAuthn token secret **without** `require_api_key` does **not** open that bind — JSON routes would otherwise stay unauthenticated. A **stderr warning** still applies when the bind is non-loopback and the key is not effectively required. Prefer loopback + reverse proxy, or `require_api_key` with a strong key in production.
- **Request body size limit:** The API rejects requests whose body exceeds **1 MB** (e.g. POST `/config`, POST `/scan`, POST `/scan_database`) with **HTTP 413 Payload Too Large**, counting **actual received bytes** (not only the `Content-Length` header) so chunked transfer cannot skip the cap. See **`api/request_body_limit.py`** and **`tests/test_security.py`**.
- **Logging policy:** API key, passwords, and connection strings must not appear in audit or application logs. **`get_logger()`** attaches **`SanitizeLogFilter`** (#1722 / ADR-0036) so every `.warning()`/`.error()`/`.info()` runs **`sanitize_log_text`**. Failure details still use **`save_failure`** (sanitized) and **`clean_error`**. See **`utils/logger.py`**, **`core/database.py`**, and **`tests/test_logger_pii_filter.py`**.
