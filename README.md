...
| **`pip` / `uvx` from PyPI** | The package **is on** [PyPI](https://pypi.org/project/data-boar/). Install with **`pip install data-boar`** or **`pipx install data-boar`**. Zero-clone CLI: **`uvx data-boar --config config.yaml`**. Lowest-friction demo: **`uvx data-boar --demo`**. The last **stable** wheel can lag **`main`** (pre-release lines such as **`1.8.0-rc`** are git-only). Contributor clone: **git clone** + **`uv sync`** — see [CONTRIBUTING.md — Repository and install identity](CONTRIBUTING.md#repository-and-install-identity-data-boar). On Debian/Ubuntu and Fedora with Python >=3.12, `pip`/`pipx` is usually frictionless; RHEL9-family and Alpine/musl currently need one extra onboarding step — see [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md). |
