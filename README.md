...
> **Try it in 30 seconds (no real data):** `data-boar --demo` (or `python main.py --demo`) → open `http://127.0.0.1:8088/pt-br/`. **Docker:** `docker run --rm -p 8088:8088 fabioleitao/data_boar:latest demo`. **Shell wrapper:** `./scripts/demo.sh` (requires `uv`). **Windows, never used a terminal?** → **[Windows guide (no Docker)](docs/QUICKSTART_WINDOWS.md)** · else [5-min QuickStart](QUICKSTART.md). Synthetic data only; loopback plaintext (`--allow-insecure-http`).
...
| **Enterprise remediation hooks** (coming: scan → map → tokenize/mask/encrypt-in-place → re-scan → audit trail) | [USE_CASE_SCAN_AND_REMEDIATE.md](docs/use-cases/USE_CASE_SCAN_AND_REMEDIATE.md) ([pt-BR](docs/use-cases/USE_CASE_SCAN_AND_REMEDIATE.pt_BR.md)) · [USE_CASE_TOKENIZED_FINDINGS.md](docs/use-cases/USE_CASE_TOKENIZED_FINDINGS.md) ([pt-BR](docs/use-cases/USE_CASE_TOKENIZED_FINDINGS.pt_BR.md)) |
...
| **`pip` / `uvx` from PyPI** | The package **is on** [PyPI](https://pypi.org/project/data-boar/). Install with **`pip install data-boar`** or **`pipx install data-boar`**. Zero-clone CLI: **`uvx data-boar --config config.yaml`**. Lowest-friction demo: **`uvx data-boar --demo`**. The last **stable** wheel can lag **`main`** (pre-release lines such as **`1.8.0-beta`** are git-only). Contributor clone: **git clone** + **`uv sync`** — see [CONTRIBUTING.md — Repository and install identity](CONTRIBUTING.md#repository-and-install-identity-data-boar). On Debian/Ubuntu and Fedora with Python >=3.12, `pip`/`pipx` is usually frictionless; RHEL9-family and Alpine/musl currently need one extra onboarding step — see [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md). |
