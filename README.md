...
| **`pip` from PyPI** | **`pip install data-boar`** when published; until then **git clone** + **`uv sync`** — see [CONTRIBUTING.md — Repository and install identity](CONTRIBUTING.md#repository-and-install-identity-data-boar). On Debian/Ubuntu and Fedora with Python >=3.12, `pip`/`pipx` is usually frictionless; RHEL9-family and Alpine/musl currently need one extra onboarding step — see [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md). |
