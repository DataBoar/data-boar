...
- **pip / `uvx` via PyPI:** o pacote **já está no** [PyPI](https://pypi.org/project/data-boar/). Instale com **`pip install data-boar`** ou **`pipx install data-boar`**. CLI sem clone: **`uvx data-boar --config config.yaml`**. Demo de menor atrito: **`uvx data-boar --demo`**. A última wheel **estável** pode ficar atrás do **`main`** (linhas pré-release como **`1.8.0-rc`** são só git). Clone de colaborador: **git clone** + **`uv sync`** — veja [CONTRIBUTING.pt_BR.md — Repositório e identidade](CONTRIBUTING.pt_BR.md#repositório-e-identidade-de-instalação-data-boar). Em Debian/Ubuntu e Fedora com Python >=3.12, `pip`/`pipx` costuma ser sem atrito; em RHEL9-family e Alpine/musl há 1 passo extra de onboarding — veja [TROUBLESHOOTING.pt_BR.md](docs/TROUBLESHOOTING.pt_BR.md).
