...
1. **Test optional extras** – job **`test-extras`** (Python **3.13** only) installs SQL extras **except** `mariadb`, plus `nosql` + `compressed` + `dataformats` (+ `shares`) so optional-connector tests run instead of skip; a skip-count ceiling fails the job if silent skips grow (issue **#1638**). The job also checks out private **[DataBoar/maestro](https://github.com/DataBoar/maestro)** into `$GITHUB_WORKSPACE/maestro` and sets **`MAESTRO_ROOT`** so consumer-side Maestro guards (`tests/test_maestro_scripts.py` and related) run instead of skip. That checkout uses repo secret **`MAESTRO_CHECKOUT_TOKEN`** (fine-grained PAT with `contents:read` on `DataBoar/maestro`; default `GITHUB_TOKEN` cannot read another private org repo). The `mariadb` extra stays out of this 3.13 job: PyPI **1.1.14** (latest stable) raises `SyntaxError` on import (`connectionpool.py` non-raw docstring); **2.0.0** is still rc-only. Restore `sql-all` / `--extra mariadb` when a stable connector imports on 3.13.
