...
L3 **transformed_rows** (real-data plane, `--export-l3`): pin of `schema/transformed_rows.schema.json` at [`sdk/transformed_rows.schema.json`](sdk/transformed_rows.schema.json) + [`sdk/transformed_rows.pin.json`](sdk/transformed_rows.pin.json) (not a second contract). Grant-scoped; stdout unless `--l3-persist`. Persist is fail-closed: POSIX owner-only `0600`; Windows `icacls` DACL (owner Full Control — **SYSTEM** and **Administrators** typically remain; this is **not** POSIX `0600`). Unproven containment → process exit **5** and the file is removed, unless `--l3-allow-unprotected` records `containment`: `not_enforced` on the stderr audit line. Other exits: **3** missing grant / Community; **4** column outside grant. Network drift canary: `DATA_BOAR_SDK_SCHEMA_CHECK=1` / workflow `sdk-schema-pin-canary.yml`. Refresh pins: `scripts/sync_sdk_schema_pin.py`. `co_columns` waits on [data-boar-sdk#41](https://github.com/DataBoar/data-boar-sdk/issues/41).
