...
1. **Licenças JWT assinadas com Ed25519** — enforcement fail-closed (`licensing.mode: enforced`). Overlay opcional na biblioteca (#48): `decode_license_jwt_hybrid` também pode checar `dbmldsa_sig` (ML-DSA-65). **O `LicenseGuard` chama `decode_license_jwt_hybrid` quando `dbmldsa_sig` está presente** — veja [LICENSING_SPEC.pt_BR.md](LICENSING_SPEC.pt_BR.md) *Overlay híbrido Ed25519 + ML-DSA-65*.
