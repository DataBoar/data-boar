...
- **connectors/redis_connector.py** (opcional) — **RedisConnector**: connect (pin SSRF/TCP), `SCAN` até `sample_limit` chaves, scanner nos **nomes concatenados**, depois amostragem de payload por TYPE quando o nome é LOW (`GET` / `HSCAN` / `LRANGE` / `SSCAN` / `ZRANGE` / `XRANGE`). Tipos não suportados → `save_failure(..., redis_value_not_sampled, json)`. Achados: `table_name="keys"`. Registrado para redis.
