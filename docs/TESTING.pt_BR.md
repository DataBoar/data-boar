...
| **test_workflow_run_scalar_guard.py**         | `run:` folded do GitHub Actions (`>` / `>-`) com `\` no shell deve falhar; bloco literal `|` com `\` deve passar. Os **`.github/workflows/`** reais precisam ficar limpos. Script **`scripts/workflow_run_scalar_guard.py`** (#1918). **`act`** opcional: [ACT_PODMAN_WORKFLOW_SMOKE.pt_BR.md](ops/ACT_PODMAN_WORKFLOW_SMOKE.pt_BR.md). |
...
- **SonarQube/SonarCloud** — quando `SONAR_TOKEN` está definido; o job do scanner usa Python **3.12** após os testes passarem. Ele **baixa** o artefato **`coverage-xml`** da célula Test em Python **3.13** (`sonar.python.coverage.reportPaths=coverage.xml` em `sonar-project.properties`). Sem esse artefato o job Sonar falha (`if-no-files-found: error` no upload). O mesmo `sonar-project.properties` na raiz do repositório é usado pela extensão no IDE e pelo scanner no CI.

**XML de cobertura (#1719):** o CI rodava o Sonar **sem** `coverage.xml`, então um ruleset de **code coverage** ficava **ligado e vazio**. A célula Test em Python **3.13** agora publica XML do pytest-cov (`[tool.coverage.run]` no `pyproject.toml`; `pytest-cov` no grupo **dev**). O `uv run pytest` local **não** gera esse XML a menos que você passe `--cov`. **Não** faça commit de `coverage.xml`.
