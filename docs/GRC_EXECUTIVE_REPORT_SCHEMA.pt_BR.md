...
1. **Metadados auditáveis:** **`report_id`** estável, **`scan_date`** em UTC, **`scanner_version`**, texto de **`scope`** alinhado ao que foi configurado (*targets*, limites, *jurisdiction hint* quando usado).
1. **Dois consumidores:** o mesmo *payload* alimenta **UI interativa** e **PDF** (resumo executivo primeiro).
1. **Mapeamento de compliance não autoritativo:** `compliance_mapping` lista **candidatos** para oficina; **jurídico / DPO** decide aplicabilidade. O produto **não** determina sozinho base legal.
...
1. Tratar **`compliance_mapping`**, o **`regulatory_impact`** de cada linha e o **`regulatory_impact_note`** das recomendações como **ponto de partida** de oficina. **Não** tratar texto modelo sobre multas ou artigos como **assessoria verificada** sem revisão jurídica.
1. Manter **PII bruta** fora deste JSON; anexos redigidos ou dashboard com controlo de acesso.
1. Versionar **`schema_version`** nas notas de release quando campos mudarem.
1. Conferir **`data_category`** (`personal` vs `sensitive`) contra o **inventário de dados** e a análise jurídica; o classificador do produto é **indício**, não decisão definitiva de LGPD art. 11.
