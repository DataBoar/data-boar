...
**Build / PyPI / About:** **`1.7.4.post10`** — PEP 440 post-release (publish counter per [ADR-0073](../adr/ADR-0073-version-scheme-octet-maturity-and-roadmap.md) § *PyPI dual counters*).

**Not in this drop:** Git tag · GitHub Release · Docker / container image (PyPI-only `.postN`).

**Theme:** **Visibility drop** — four fixes make **reportable** what was previously **silent**. None of them change what the scanner **reads** on the hot path; they change what operators and stakeholders **see** in exports and `scan_failures`. Part **B** of [#1348](https://github.com/DataBoar/data-boar/issues/1348) (Redis type-aware sampling) and [#1354](https://github.com/DataBoar/data-boar/issues/1354) (`use_content_type` dispatch) remain **1.8.x** — they change **read/decide** behaviour, not only reporting.

**Honesty note:** [#1349](https://github.com/DataBoar/data-boar/issues/1349) (OS compatibility matrix ↔ `TECH_GUIDE` alignment) shipped in the same merge ([#1357](https://github.com/DataBoar/data-boar/pull/1357)) but is **docs-only** — it does **not** change the installable wheel and is **not** counted in `N`. See [CHANGELOG.md](../../CHANGELOG.md) post10 notes.
...
## Appendix — Fix set used for `N` (`N=4`) (git-literal)

**Boundary:** post9 release commit `9fa991c2` (`chore(release): 1.7.4.post9 PyPI publish counter`).

```bash
git log 9fa991c2..b35e96b9 --format='%h %s' \
  | rg 'b35e96b9'
# COUNT=4 discrete fixes in one wheel commit  →  maturity_build = 257 + 4 = 261
```

Commits / issues counted toward **`N=4`** (wheel-affecting packages only):

1. **`.258` — [#1353](https://github.com/DataBoar/data-boar/issues/1353):** executive PDF/DOCX exported raw Markdown (`**`, `` ` ``, `####`) to board/DPO readers; shared `report/executive_markdown_render.py` + renderer updates ([#1357](https://github.com/DataBoar/data-boar/pull/1357), `b35e96b9`). Corrects a **delivery-surface defect** introduced with post9 session export paths.
2. **`.259` — [#1348](https://github.com/DataBoar/data-boar/issues/1348) Part A:** Redis `GET` on non-string keys raised `WRONGTYPE`, swallowed as `None` — indistinguishable from connection failure; now reports `redis_value_not_sampled` with per-type counts and `sample_limit`-derived key classification cap (Part B: type-aware reads → **1.8.x**).
3. **`.260` — [#1354](https://github.com/DataBoar/data-boar/issues/1354) Part A:** compressed extension vs magic mismatch (e.g. `sample4.tar.bz2`) returned `None` like a plain file — now `archive_type_mismatch` in `scan_failures` without expanding the archive (Part B: `use_content_type` dispatch → **1.8.x**).
4. **`.261` — [#1340](https://github.com/DataBoar/data-boar/issues/1340):** raise `pypdf` **floor** to `>=6.14.2` so a future dependency resolve cannot reinstall the four DoS CVE builds (post9 pinned `6.14.2` but left floor at `6.13.0`).

**Not counted toward `N`:** merge commits; release-prep commits for post10; [#1349](https://github.com/DataBoar/data-boar/issues/1349) OS compatibility matrix docs only (same PR #1357); [#1355](https://github.com/DataBoar/data-boar/pull/1355) lab-smoke seeds + `.gitignore` (`aa06fcbe`); post9 published-status doc stamp ([#1351](https://github.com/DataBoar/data-boar/pull/1351)); backlog post1–post7 PyPI timestamps ([#1352](https://github.com/DataBoar/data-boar/pull/1352)).

---

## Highlights (why post10)

- **Executive deliverables:** PDF and DOCX session exports now render headings, bold, and monospace instead of leaking Markdown tokens ([#1353](https://github.com/DataBoar/data-boar/issues/1353)).
- **Honest negatives:** Redis WRONGTYPE and archive extension/magic mismatch no longer look like “clean scan, no PII” ([#1348](https://github.com/DataBoar/data-boar/issues/1348) A, [#1354](https://github.com/DataBoar/data-boar/issues/1354) A).
- **Supply chain:** `pypdf` minimum version aligned with post9 pin ([#1340](https://github.com/DataBoar/data-boar/issues/1340)).
...
```bash
pip install 'data-boar==1.7.4.post10'
```
