...
**Promoted operator hygiene (post-triage, not “Gemini as authority”):** dated checklists consolidated under **`docs/ops/today-mode/`** with **`PUBLISHED_SYNC.md`** and **`CARRYOVER.md`** — see [ADR 0006](../adr/ADR-0006-operator-today-mode-layout-and-published-sync.md).

**Source texts (maintainer machine, gitignored):** `docs/feedbacks, reviews, comments and criticism/` — same drop zone as **Corporate-Entity-C / WRB**-style reviews (rule **`.cursor/rules/operator-feedback-inbox.mdc`**, token **`feedback-inbox`**). Gemini exports used for §6: `Gemini - revisao e recomendações … EN …`, `… pt_BR …`, `… compliance config samples yaml …`, `Gemini - raciocinio profundo …` (filenames show `2006_03_26` typo; treat as **2026-03-26**). **Do not** commit that folder; this plan holds the **distilled triage** only.
...
| **Storage** | Prefer the agreed folder **`docs/feedbacks, reviews, comments and criticism/`** (gitignored) for full Gemini / Corporate-Entity-C text; alternatively **`docs/private/gemini_bundles/`** or journals — **never** paste webhooks, credentials, or client-specific data into **tracked** Markdown. |
...
| **G-26-04** | **Package name vs brand:** alinhar PyPI/docs a **Data Boar** / `data-boar`. | Feito: `pyproject` + docs; publicação PyPI quando houver release. | Cold | Operator success | ✅ |
...
| **G-13-02** | **CI fail-open/fail-closed:** pipeline lacks explicit documentation on behavior during dependency fetch timeouts. | Check `.github/workflows/ci.yml` error handling; confirm `pip-audit` / `uv` timeout behavior is documented. | Warm | Trust | **Resolvido** — bloco "Failure posture" adicionado ao cabeçalho de `.github/workflows/ci.yml` documentando comportamento fail-closed por padrão: step falha → job falha; `fail-fast: false` = visibilidade paralela, não passagem silenciosa; falhas de rede em `uv sync` e `pip-audit` encerram o job. |
...
| **G-13-05** | **PII memory limits** for large repo diffs not specified in PII guardrails docs. | Check `PII_VERIFICATION_RUNBOOK.md` — document `--max-diff-size` or similar limit. | Warm | Trust | **Resolvido** — `--max-diff-size-mb` (default 500 MB) adicionado a `scripts/pii_history_guard.py`; aborta com mensagem clara se o diff ultrapassar o limite; documentado no argumento `--help`. |

### 6.5b pt-BR locale

| ID | Item | Verify how | Urgency | Aligns §5? | Status |
| -- | ---- | ---------- | ------- | ----------- | ------ |
| **G-13-06** | **"Ficheiro"** in terminal output examples instead of "arquivo". | `uv run pytest tests/test_docs_pt_br_locale.py`; also grep terminal/output text in templates. | Warm | Compliance narrative | **Resolvido** — única ocorrência funcional de "ficheiro" em `docs/ops/AUDIT_PROTOCOL.md` corrigida para "arquivo". Restantes ocorrências são warnings _contra_ uso de "ficheiro" (estão corretas). |
...
| ID | Item | Verify how | Urgency | Aligns §5? | Status |
| -- | ---- | ---------- | ------- | ----------- | ------ |
| **G-13-09** | **min_confidence missing** in GDPR and other compliance YAML examples → engine may default to 1.0 causing silent FN. | Check `compliance-sample-eu_gdpr.yaml`; confirm engine default for min_confidence when absent. | Hot | Trustworthy scans | **Resolvido** — bloco "Detection thresholds" adicionado em `eu_gdpr.yaml` e `lgpd.yaml`; documentado que score ML é interno (sem campo por-termo). |
| **G-13-10** | **Minor detection keywords in English only** → critical FN for Brazilian pt-BR documents (e.g. "data de nascimento" not matched). | Check `compliance-sample-us_co_cpa_minors.yaml` and `core/detector.py` minor detection keyword lists. | Hot | Trustworthy scans | **Resolvido** — `_detect_possible_minor` += `dt_nasc`, `dt_nascimento`, `dtnasc`, `menor_de_idade`, `idade_anos`, `anos`, `faixa_etaria`, `menores`, `crianca`, `criança`, `adolescente`. |
| **G-13-11** | **CPF/CNPJ samples lack checksum validation hint** → docs may imply format-only matching, risking FP flood. | Engine DOES validate checksums (confirmed in tests); YAML samples should note this. Add comment in `compliance-sample-lgpd.yaml`. | Warm | Trustworthy scans | **Resolvido** — comentário Mod-11 em `lgpd.yaml`; corpus expandido em `test_brazilian_cpf.py` (12 CPFs válidos + 4 CNPJs + `test_false_positive_rate_zero_on_invalid_shapes`; 29 testes + 32 subtests verdes); `homelab_synthetic/person_a_notes.txt` enriquecido com exemplos válidos e inválidos. |
| **G-13-12** | **financial_routing_number regex too broad** → matches generic UUIDs/sequential IDs. | Review `compliance-sample-pci_dss.yaml` routing regex; add context bound or warning comment. | Warm | Trustworthy scans | **Resolvido** — bloco "NOTE — financial routing / ABA routing number regex" adicionado a `compliance-sample-pci_dss.yaml` documentando risco de FP alto em padrões de 9 dígitos e guia de mitigação. |
| **G-13-13** | **Override behavior (merge vs replace) unclear** when health data sample loaded alongside base LGPD. | Document in USAGE.md `override_behavior` section; confirm in code. | Warm | Operator success | **Resolvido** — seção "Merge behavior: additive by design" adicionada a `COMPLIANCE_FRAMEWORKS.md`: `regex_overrides_file` e `ml_patterns_file` são ADITIVOS aos built-ins; `recommendation_overrides` usa first-match por ordem de lista. |

### 6.5d Cross-cutting

| ID | Item | Verify how | Urgency | Aligns §5? | Status |
| -- | ---- | ---------- | ------- | ----------- | ------ |
| **G-13-14** | **boar_fast_filter + malformed UTF-8**: does Rust filter silently drop malformed files before Python engine can flag them for review? | Check `rust/boar_fast_filter/src/` encoding error handling; add note to TECH_GUIDE on encoding boundary behavior. | Hot | Trust | **Resolvido** — nova seção "Rust acceleration module" em `TECH_GUIDE.md` documenta boundary de encoding: Python decodifica com `errors="replace"`, Rust opera só em `str` já decodificada; arquivos não são silenciosamente descartados. |
| **G-13-15** | **SQLite corruption on aborted scan** (`qa_kill_scan.py`): audit log may leave open transaction or corrupted journal. | Read `database/` session management; test with kill during scan. | Hot | Trust/evidence | **Resolvido** — `tests/test_scan_abort_integrity.py` (3 testes: integrity_check, uncommitted rows absent, LocalDBManager reopen); garante ACID recovery via SQLite journal após SIGKILL. |
| **G-13-16** | **Jurisdiction collision tie-breaker** (LGPD + GDPR both match): no documented logic for which wins in report. | USAGE/COMPLIANCE_METHODOLOGY — add "multi-jurisdiction" section explaining current behavior (additive, not exclusive). | Warm | Compliance narrative | **Resolvido** — seção "Multi-jurisdiction behavior (quando LGPD + GDPR ambos batem)" adicionada a `COMPLIANCE_FRAMEWORKS.md`: comportamento é additive/union, sem tie-breaker automático; ordem em `recommendation_overrides` determina texto da recomendação; remete a ADR 0025 + ADR 0038. |
