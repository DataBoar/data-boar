...
- **Status rows counted:** 212  (Done: 133 | Incomplete: 79)
...
- **Wheelhouse pan-ABI matrix via GitHub Releases (`#1182`, seed 2026-07-12 / v1 2026-07-29):** Target = third-party compiled deps **`cp312` + `cp313` + `cp314`** × `(manylinux|musllinux)` × `(x86_64|arm64)` × **CPU baseline (`x86-64-v1` where needed)**; **`boar_fast_filter`** stays **ONE `cp38-abi3` wheel per `(libc×arch)`** ([#782](https://github.com/DataBoar/data-boar/issues/782) / GAP-001) — **not on PyPI**; wheelhouse is the only channel today. **Verified release:** [`wheelhouse-x86-64-v1-2026-07-29`](https://github.com/DataBoar/data-boar-site/releases/tag/wheelhouse-x86-64-v1-2026-07-29) on **`DataBoar/data-boar-site`** (two-step `pipx` + offline ML swap + `boar_fast_filter` inject; **46** assets incl. `mariadb` both libcs × cp312/313/314). **Docs rollout:** [#1365](https://github.com/DataBoar/data-boar/issues/1365) → `TROUBLESHOOTING` + `OS_COMPATIBILITY_TESTING_MATRIX` + [PLAN_WHEELHOUSE_DISTRIBUTION.md](PLAN_WHEELHOUSE_DISTRIBUTION.md). **`mariadb` glibc recipe pinned** ([#1367](https://github.com/DataBoar/data-boar/issues/1367) — Connector/C from source); aarch64 still [#1366](https://github.com/DataBoar/data-boar/issues/1366). **cp314 = signal-only** in CI gating. **Remaining #1182:** arm64 slice + PEP 503 `simple/` index with **`+x86v1`** local versions (index alone does not beat PyPI on version tie).
- **Database compat matrix — sparse A×B (`#1237` / `#1302`, plan 2026-07-22):** Axis A = platform hosts (lab nodes — FS/CSV/SQLite); axis B = engines on a Podman farm host (pg/mysql/mariadb/mssql/oracle/sqlite, mongo, redis) + separate cloud-lane; sparse intersections only for native drivers (pymssql/psycopg2/oracledb thick). A light lab client → remote farm validates [#1237](https://github.com/DataBoar/data-boar/issues/1237) latency. Synthetic fixtures only. Tracking plan: [PLAN_DATABASE_COMPAT_MATRIX.md](PLAN_DATABASE_COMPAT_MATRIX.md). Cross-refs [#1171](https://github.com/DataBoar/data-boar/issues/1171), [#1182](https://github.com/DataBoar/data-boar/issues/1182), [#782](https://github.com/DataBoar/data-boar/issues/782)/GAP-001.
...
| [1298](https://github.com/DataBoar/data-boar/issues/1298) | CRITICAL_MODULES incomplete — connectors/licensing/PII outside anchor | Security / integrity | [PLAN_INTEGRITY_HARDENING.md](PLAN_INTEGRITY_HARDENING.md) | **`[H0][U1]`** [P1] — field false-negative; glob allowlist + CLI `--version` / `--validate-config` surfacing |
| [1182](https://github.com/FabioLeitao/data-boar/issues/1182) | Wheelhouse distribution (pan-ABI: cp312/313/314 + abi3 ext) | Packaging / distro | [PLAN_WHEELHOUSE_DISTRIBUTION.md](PLAN_WHEELHOUSE_DISTRIBUTION.md) | **`[H1][U1]`** [P1] — **x86-64-v1** release + user docs ([#1365](https://github.com/DataBoar/data-boar/issues/1365)) ✅; remainder: arm64 + PEP 503 `simple/` with `+x86v1` local versions |
| [1237](https://github.com/DataBoar/data-boar/issues/1237) · [1302](https://github.com/DataBoar/data-boar/issues/1302) | Database compat matrix (sparse A×B; remote latency + connector kwargs) | Lab / connectors | [PLAN_DATABASE_COMPAT_MATRIX.md](PLAN_DATABASE_COMPAT_MATRIX.md) | **`[H1][U1]`** [P1]/[P2] — plan scaffold; Podman farm + remote light client; synthetic fixtures; complements [#1171](https://github.com/DataBoar/data-boar/issues/1171) platform epic |
| [1338](https://github.com/DataBoar/data-boar/issues/1338) | Benchmark safe axis — recall gate beside speed | Performance / quality | [PLAN_BENCHMARK_SAFE_AXIS.md](PLAN_BENCHMARK_SAFE_AXIS.md) | **`[H1][U1]`** — blocks [#1337](https://github.com/DataBoar/data-boar/issues/1337); exact synthetic corpus parity |
| [1337](https://github.com/DataBoar/data-boar/issues/1337) | SQL sampling dedup / recall-safe optimization | Performance / SQL | [PLAN_SAMPLING_DEDUP_BEFORE_CAP.md](PLAN_SAMPLING_DEDUP_BEFORE_CAP.md) | **`[H1][U2]`** — client dedup before cap; blocks on #1338 ✅ |
...
**Purpose:** Single **navigation anchor** in this file for **integrity, tamper-evidence, and trust-downgrade** work — **links + scope + non-goals**. Implementation detail for the expanded behaviour-critical allowlist: [PLAN_INTEGRITY_HARDENING.md](PLAN_INTEGRITY_HARDENING.md) ([#1298](https://github.com/DataBoar/data-boar/issues/1298)).

**Scope (what this index covers):**

- **Behaviour-critical allowlist expansion (glob + CLI surfacing):** `connectors/`, `core/licensing/`, PII redaction, WebAuthn — [PLAN_INTEGRITY_HARDENING.md](PLAN_INTEGRITY_HARDENING.md) (**Phase 1** #1298).
