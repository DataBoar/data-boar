...
- **Wheelhouse pan-ABI matrix via GitHub Releases (`#1182`, seed 2026-07-12 / scope rewrite 2026-07-22):** Target = third-party compiled deps **`cp312` + `cp313` + `cp314`** × `(manylinux|musllinux)` × `(x86_64|arm64)` (fill upstream gaps; e.g. sklearn cp314 musllinux); **`boar_fast_filter`** stays **ONE `cp38-abi3` wheel per `(libc×arch)`** — not per-cpXXX ([#782](https://github.com/DataBoar/data-boar/issues/782) / GAP-001). Seed release **`wheelhouse-2026-07-12`** on **`DataBoar/data-boar-site`** proven with `pipx … --find-links` + Alpine `--demo`. Tracking plan: [PLAN_WHEELHOUSE_DISTRIBUTION.md](PLAN_WHEELHOUSE_DISTRIBUTION.md). **cp314 = signal-only** in CI gating. **Gate anti-link-morto:** do **not** update `TROUBLESHOOTING` / `OS_COMPATIBILITY_TESTING_MATRIX` URLs until the hosted command contract is finalized and verified.
...
| [1182](https://github.com/FabioLeitao/data-boar/issues/1182) | Wheelhouse distribution (pan-ABI: cp312/313/314 + abi3 ext) | Packaging / distro | [PLAN_WHEELHOUSE_DISTRIBUTION.md](PLAN_WHEELHOUSE_DISTRIBUTION.md) | **`[H1][U1]`** [P1] — cp314 musllinux seed done; expand full matrix `cp312`+`cp313`+`cp314` × libc × arch for non-abi3 deps; `boar_fast_filter` abi3-only (#782 / GAP-001); hosted index (`simple/`) before broad doc rollout; cp314 signal-only in CI |
