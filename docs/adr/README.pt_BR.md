...
- [ADR 0045](0045-dependabot-ignore-ebcdic-2x-blocked-by-extract-msg.md) (EN) — Dependabot **ignora** `ebcdic >= 2.0.0` no bloco `uv` enquanto **`extract-msg` 0.55.0** fixa `ebcdic<2,>=1.1.1` no `requires_dist`; resolve a falha determinística da CI run `25607342909` (PR #346) sem afrouxar o guard `tests/test_dependency_artifacts_sync.py`. Reabrir quando o upstream relaxar a restrição.
