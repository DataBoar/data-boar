...
- [ADR 0045](0045-chardet-pinned-by-cyclonedx-bom.md) (EN) — **`chardet` major bumps** ficam ignorados no Dependabot (`update-types: ["version-update:semver-major"]`) enquanto `cyclonedx-bom` mantiver o pin upstream `chardet>=5.1,<6.0`; PR #347 fechado como **superseded — blocked upstream**; condição de saída registrada na ADR e no comentário do `.github/dependabot.yml`.
