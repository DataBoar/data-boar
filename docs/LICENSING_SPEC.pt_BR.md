...
| `dbmldsa_sig` | string | Overlay opcional ML-DSA-65 (#48). **Não** é algoritmo JOSE. Ver [Overlay híbrido Ed25519 + ML-DSA-65](#overlay-híbrido-ed25519--ml-dsa-65-48). |

### Overlay híbrido Ed25519 + ML-DSA-65 (#48)

O `alg` JOSE permanece **EdDSA** (Ed25519). O License Studio pode anexar o claim `dbmldsa_sig` (`CLAIM_MLDSA_SIG` em `core/licensing/verify.py`): base64url de uma assinatura **ML-DSA-65** sobre `header_b64` + `.` + base64url do JSON do payload **sem** esse claim.

| Superfície | O que executa |
| ---------- | ------------- |
...
| `LicenseGuard` | Continua chamando **somente** `decode_license_jwt`. O scan enforced **não** exige ML-DSA hoje. |
...
- Testes: `tests/test_licensing_hybrid_mldsa_verify.py` (pula se o OpenSSL não tiver ML-DSA: `backend.mldsa_supported()`).
- **Barra de forja:** quem forja Ed25519 pode **omitir** `dbmldsa_sig` e o caminho só-Ed25519 ainda aceita o token. A verificação híbrida **não** sobe o enforcement de produção até uma política futura exigir ML-DSA (watch pós-quântico do ADR-0063).
...
| **Emissão**                 | O emissor privado ([`DataBoar/license-studio`](https://github.com/DataBoar/license-studio) — só operador) emite **templates** de token diferentes por SKU; o log de auditoria registra o programa/tier.              |             |                                                                                                      |
...
| `dbmax_workers` (int)                                 | **Implementado (#551/#853)** — limite de **workers de scan em paralelo** (≈ alvos concorrentes). Padrões por faixa: Community **2** · Pro **4** · Pro+ **8** (dirigido por claim) · Enterprise **ilimitado** — escada canônica em [SUBSCRIPTION_TIERS.pt_BR.md](SUBSCRIPTION_TIERS.pt_BR.md) (não é lista de preços).                                                                                                                                                                                                                                                                                                                                                     |
...
1. **Open core / community** (sem token pago): quando o enforcement está ativo para um **build** que embute padrões de faixa, alinhe com a política de **1–2 workers** e **alvos limitados** — ver [SUBSCRIPTION_TIERS.pt_BR.md](SUBSCRIPTION_TIERS.pt_BR.md).
