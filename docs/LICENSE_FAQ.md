...
1. **Signed Ed25519 JWT licenses** — fail-closed enforcement (`licensing.mode: enforced`). Optional library overlay (#48): `decode_license_jwt_hybrid` can also check `dbmldsa_sig` (ML-DSA-65). **`LicenseGuard` calls `decode_license_jwt_hybrid` when `dbmldsa_sig` is present** — see [LICENSING_SPEC.md](LICENSING_SPEC.md) *Hybrid Ed25519 + ML-DSA-65 overlay*.
