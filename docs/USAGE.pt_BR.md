...
- **Auth do audit trail (#1190):** cada execução gera `api.api_key` aleatória (32 bytes) e `api.rbac.api_key_roles: [audit_logs.read]` (não `admin`). O banner imprime a chave e um `curl -H "X-API-Key: …"` pronto para `GET /logs/{session_id}`. Sem a chave permanece **401** — o demo **não** bypassa o default-deny do RBAC.
- **Saída:** banner no console com caminho do workspace, URL do dashboard (porta padrão **8088**), chave de API e exemplo `curl`. A árvore temporária é removida ao encerrar o processo.
