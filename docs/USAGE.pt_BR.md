...
| `GET`   | `/status`                           | Retorna `running`, `current_session_id`, `findings_count`, além de `runtime_trust`, **`detection_prefilter`** (#1411), `dashboard_transport`, `enterprise_surface` e **`maturity_assessment_integrity`** (resumo HMAC das respostas do POC quando configurado). |
