...
- **RBAC (opcional — fase 2, Pro+ `dashboard_rbac`):** Com **`api.rbac.enabled: true`** e tier efetivo que permita **`dashboard_rbac`** (`licensing.effective_tier` no YAML ou claim **`dbtier`** no JWT com enforcement — mesma regra das outras funções com tier), rotas protegidas exigem **papéis**: `admin` (tudo), `dashboard`, `scanner`, `reports_reader`, `config_admin`, e **`audit_logs.read`** para **`GET /logs`** / **`GET /logs/{session_id}`**. A **chave global de API** (quando presente e correta) recebe **`api.rbac.api_key_roles`**; sessões WebAuthn usam a coluna **`roles_json`** na linha correspondente de `webauthn_credentials` (array JSON de nomes de papel), ou **`api.rbac.default_roles`** quando **`roles_json`** estiver vazio. **401** sem principal; **403** com papéis insuficientes. Quando o RBAC **não** estiver ativo, `/logs` segue a mesma postura de `/findings` / `/report` (ainda sujeito a `api.audit_logs.enabled` / directory e a **`api.require_api_key`** quando essa flag estiver ligada). **`GET /status`** e **`GET /health`** incluem **`enterprise_surface.access_surface.rbac`** (`enabled` vs `not_implemented`). Tier **Community** não habilita RBAC no produto; use **`require_api_key`**, proxy reverso ou rede. Veja o plano interno **`PLAN_DASHBOARD_REPORTS_ACCESS_CONTROL.md`** (na lista *Internal and reference* em `docs/README.md`; GitHub **#86**); **SSO/OIDC** empresarial permanece na fase **3**.
