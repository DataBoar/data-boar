...
- **`GET /health` (sempre público):** Sonda liveness/readiness; JSON com `status`, resumo **público** de `license`, `dashboard_transport`, `enterprise_surface` e **`trust_state`** / **`trust_reasons`** / **`output_confidence`**. **Não** é necessário (nem checado) `X-API-Key` / Bearer.
