...
**Sample configuration and examples:** We provide config-file examples so you can align with more regulations without changing code (including **per-region YAML profiles** under **`compliance-samples/`**, e.g. **Russia 152-FZ** — see [List of samples and links](#list-of-samples-and-links)):
...
**Compliance samples (one profile per regulation):** Sample configs for **UK GDPR**, **EU GDPR**, **EU NIS2**, **Benelux**, **PIPEDA**, **POPIA**, **APPI**, **PCI-DSS**, **Russia (Federal Law 152-FZ)**, and other regions are in [compliance-samples/](compliance-samples/). Each sample is a single YAML file with regex patterns, ML terms, and recommendation overrides for that framework. **NIS2** is a **cybersecurity** directive (not a personal-data law); it **complements GDPR** because NIS2 incidents often trigger GDPR breach notification in parallel — pair [compliance-sample-eu_nis2.yaml](compliance-samples/compliance-sample-eu_nis2.yaml) with the EU GDPR sample. See the [Compliance samples](#compliance-samples) section below for the full list, what goes in which file, and how to use them. For **language and target audience** (e.g. PIPEDA → EN + FR for Canada; **152-FZ** → EN + RU terms in the Russian sample), see the [compliance-samples README](compliance-samples/README.md).
...
Laws, guidance, and real-world identifier usage evolve in many sampled jurisdictions—not only those with the fastest statutory churn. **Statutes and official texts are also reworded or replaced over time**, so YAML that “matched last year” may need updates even when politics are stable. Plan to **refresh samples you rely on** (regex, terms, overrides) on a cadence that fits your risk—**quarterly or before a minor release** is a reasonable default for high-stakes profiles; see [Sample maintenance](compliance-samples/README.md#sample-maintenance) in the compliance-samples README. The **152-FZ** sample header calls out especially frequent legal churn; treat it as a template for documenting review risk in other jurisdictions.

### List of samples and links

| Regulation / region           | Sample file                                                                                                 | Purpose                                                                                             |
| -------------------           | -----------                                                                                                 | -------                                                                                             |
| **LGPD (Brazil)**             | [compliance-sample-lgpd.yaml](compliance-samples/compliance-sample-lgpd.yaml)                               | Bilingual PT-BR + EN terms; RG/CEP regex; **precise geolocation** regex (same geometry as VCDPA / EU GDPR sample); Brazilian deployments.                                    |
...
| **Brazil (FELCA — Lei 15.211/2025)** | [compliance-sample-brazil_felca.yaml](compliance-samples/compliance-sample-brazil_felca.yaml) | Estatuto Digital da Criança e do Adolescente (digital platforms and minors); inventory **norm_tag** and ML terms; **pair with** [LGPD sample](compliance-samples/compliance-sample-lgpd.yaml); **not** age verification (see FELCA bullet under [Auditable and management standards](#auditable-and-management-standards-supporting-role) and [MINOR_DETECTION.md](MINOR_DETECTION.md)). |
| **Brazil (ANPD Res. 2/2022 — Children's data)** | [compliance-sample-brazil_anpd_res2_2022.yaml](compliance-samples/compliance-sample-brazil_anpd_res2_2022.yaml) | ANPD Resolução CD/ANPD 2/2022 implementing LGPD Art. 14; parental consent (verificável, específico, revogável); behavioural profiling **prohibition** (norm_tag: `perfilamento proibido — menores`); educational data restrictions; in force 2023; any entity (not only platforms); complements FELCA + LGPD; PT-BR + EN. |
| **Brazil (EAD / education — LGPD Art. 14 + ECA)** | [compliance-sample-br_ead_lgpd_art14.yaml](compliance-samples/compliance-sample-br_ead_lgpd_art14.yaml) | Distance-learning and school/LMS inventory: RA / matrícula, `.edu.br` mail, **contextual** educational CPF fields (not a bare CPF matcher); ECA (Lei 8.069/1990) + LGPD Art. 14; pair with LGPD + ANPD Res. 2/2022 + FELCA; **not** age verification or MEC/ANPD certification; PT-BR + EN. |
| **Brazil (health sector — LGPD Art. 11)** | [compliance-sample-brazil_saude.yaml](compliance-samples/compliance-sample-brazil_saude.yaml) | Health-sector identifiers (CRM/CRF/CRO/COREN, CNS, ANVISA RMS, ANS) and health-data lexicon; **pair with** [LGPD sample](compliance-samples/compliance-sample-lgpd.yaml); heuristic regex — not CRM checksum validation. |
| **Brazil (BACEN Res. 4893/2021 — Cybersecurity)** | [compliance-sample-brazil_bacen4893.yaml](compliance-samples/compliance-sample-brazil_bacen4893.yaml) | BACEN cybersecurity policy for banks, fintechs, and payment institutions; Pix keys, account numbers, card tokens, NSU regex; incident notification 4 business days to BACEN (Art. 11); IT outsourcing 3-tier rules (Art. 16); cryptographic controls; in force February 2022; pairs with LGPD + PCI-DSS samples; PT-BR + EN. |
| **UK GDPR**                   | [compliance-sample-uk_gdpr.yaml](compliance-samples/compliance-sample-uk_gdpr.yaml)                         | UK post-Brexit, ICO-aligned; norm_tag and recommendation overrides; **precise geolocation** regex (same geometry as VCDPA / EU GDPR sample).                                 |
...
| **EU NIS2 (Directive 2022/2555)** | [compliance-sample-eu_nis2.yaml](compliance-samples/compliance-sample-eu_nis2.yaml)                         | Network and Information Security; essential/important entities; cybersecurity measures (Art. 21); incident notification 24h+72h to CSIRT/competent authority (Art. 23); supply chain security; vulnerability disclosure (Art. 12); credentials/keys/MFA-secrets detection; pairs with EU GDPR sample for full EU coverage; **not** a personal-data law (complements GDPR when incidents also trigger breach notification); transposition deadline 17/10/2024 (application depends on national transposition); EN. |
| **Benelux (BE, NL, LU)**      | [compliance-sample-benelux.yaml](compliance-samples/compliance-sample-benelux.yaml)                         | EU GDPR base + national IDs (BSN, NISS, LU); national DPA overrides; EN + NL/FR/DE (Belgium's third official language / Ostbelgien). |
| **Germany (BDSG)**            | [compliance-sample-germany_bdsg.yaml](compliance-samples/compliance-sample-germany_bdsg.yaml)               | BDSG as GDPR national supplement; contextual Steuer-ID / employee / biometric / scoring labels (no nude 11-digit matcher); §26 employee data **contested** (CJEU C-34/21); §31 scoring; 16 Land DPAs + BfDI; pair with EU GDPR; EN + DE. |
| **HIPAA / PHI (US Health)**   | [compliance-sample-us_hipaa_phi.yaml](compliance-samples/compliance-sample-us_hipaa_phi.yaml)               | Privacy Rule (45 CFR Part 164) + Security Rule + Breach Notification Rule + HITECH 2009; HHS OCR; NPI + DEA + MRN + ICD-10 regex; **precise geolocation** (geographic data < state = PHI Safe Harbor identifier #3); 95+ ML terms; covered entity / BAA / minimum necessary / TPO / ePHI safeguards; psychotherapy notes + 42 CFR Part 2 + GINA; 60-day breach notification; de-identification (Safe Harbor / expert determination); EN. |
| **FERPA (US Education)**      | [compliance-sample-us_ferpa.yaml](compliance-samples/compliance-sample-us_ferpa.yaml)                         | 20 U.S.C. § 1232g; FPCO; education-record inventory signals; SSN + contextual student ID + DOB regex; **precise geolocation** (4 patterns); LMS/SIS, transcript, directory information, parent/guardian and school-official vocabulary; pair with COPPA for under-13 web; EN. |
| **CMMC / CUI (US Defense)**   | [compliance-sample-us_cmmc_cui.yaml](compliance-samples/compliance-sample-us_cmmc_cui.yaml)                 | CUI markings/banners, category labels, dissemination controls (NOFORN, REL TO, …); ITAR/EAR/USML/CCL as **inventory/triage signals**; ~60 ML terms; **supports CMMC evidence / CUI discovery** — does **not** certify CMMC, assess a CMMC level, or determine ITAR/EAR applicability; EN. |
| **PIPEDA (Canada)**           | [compliance-sample-pipeda.yaml](compliance-samples/compliance-sample-pipeda.yaml)                           | Personal information, consent; Canadian identifiers (e.g. SIN); **precise geolocation** regex (same geometry as VCDPA / EU GDPR sample); EN + FR.                            |
| **Quebec (Law 25)**          | [compliance-sample-canada_qc_law25.yaml](compliance-samples/compliance-sample-canada_qc_law25.yaml)         | Act respecting the protection of personal information in the private sector (QC, modernized 2023); CAI; complements federal PIPEDA for QC private sector; **precise geolocation** (4 patterns); EN + FR. |
| **POPIA (South Africa)**      | [compliance-sample-popia.yaml](compliance-samples/compliance-sample-popia.yaml)                             | POPIA Act 4/2013; Information Regulator; SA ID regex; **precise geolocation** (same geometry as VCDPA / EU GDPR); special personal information (s.26), children's data (s.34), cross-border transfers (s.72); EN. |
| **APPI (Japan)**              | [compliance-sample-appi.yaml](compliance-samples/compliance-sample-appi.yaml)                               | Personal information and sensitive personal information (要配慮個人情報); My Number; **precise geolocation** regex (位置情報; same geometry as VCDPA / EU GDPR); EN + Japanese terms; PPC alignment; 2022 revision. |
...
| **Philippines (DPA)**         | [compliance-sample-philippines_dpa.yaml](compliance-samples/compliance-sample-philippines_dpa.yaml)         | RA 10173 + NPC IRR 2016; NPC; SSS/GSIS/TIN/PhilSys regex; **precise geolocation**; SPI categories (race, health, genetic, criminal, government IDs); 72 h breach notification; EN. |
| **Australia (Privacy Act)**   | [compliance-sample-australia_privacy.yaml](compliance-samples/compliance-sample-australia_privacy.yaml)     | Privacy Act 1988 (Cth; 2024 Amendment Act), OAIC, APPs; TFN/Medicare/ABN/Passport regex; **precise geolocation** (same geometry as VCDPA / EU GDPR); NDB scheme; sensitive information (s.6); EN. |
| **Singapore (PDPA)**          | [compliance-sample-singapore_pdpa.yaml](compliance-samples/compliance-sample-singapore_pdpa.yaml)           | PDPA 2012 (amended 2020/2021), PDPC; NRIC/FIN/UEN regex; **precise geolocation** (same geometry as VCDPA / EU GDPR); DNC, mandatory breach notification; EN.       |
| **Malaysia (PDPA + Amd. 2024)** | [compliance-sample-malaysia_pdpa.yaml](compliance-samples/compliance-sample-malaysia_pdpa.yaml)             | PDPA 2010 + Act A1727 (phases 1 Jan / 1 Apr / 1 Jun 2025, all in force); Commissioner; Data Controller/Processor; DPO; formatted MyKad + contextual labels (no bare 12-digit matcher); biometric as sensitive; precise geolocation (JSON lat/lng + CSV pair); EN. |
| **Thailand (PDPA)**          | [compliance-sample-thailand_pdpa.yaml](compliance-samples/compliance-sample-thailand_pdpa.yaml)              | Personal Data Protection Act B.E. 2562 (2019), PDPC; personal data / sensitive personal data; **precise geolocation** (4 patterns); EN + Thai. |
| **Vietnam (PDPD)**           | [compliance-sample-vietnam_pdpd.yaml](compliance-samples/compliance-sample-vietnam_pdpd.yaml)               | Decree 13/2023/NĐ-CP (effective 1 July 2024); Ministry of Public Security framing; CCCD / legacy CMND patterns; **precise geolocation** (4 patterns); EN + Vietnamese; extraterritorial scope note in sample header. |
| **South Korea (PIPA)**        | [compliance-sample-south_korea_pipa.yaml](compliance-samples/compliance-sample-south_korea_pipa.yaml)       | PIPA (개인정보 보호법), PIPC; contextual RRN + 사업자등록번호 + passport regex; **precise geolocation** (4 patterns); 95+ ML terms (EN + Korean): sensitive info, overseas transfer, entrustment, biometrics, CCTV; sector / “three-data” note in sample header. |
| **UAE (PDPL)**                | [compliance-sample-uae_pdpl.yaml](compliance-samples/compliance-sample-uae_pdpl.yaml)                       | Decree-Law 45/2021 + Cabinet Regulations 2023; UAE Data Office (UAEDO/TDRA); Emirates ID regex; **precise geolocation**; sensitive categories (health, biometric, genetic, racial, religious, criminal, financial, children's data); Free Zone note (DIFC/ADGM); EN + AR. |
| **Egypt (PDPL Law 151/2020)** | [compliance-sample-egypt_pdpl.yaml](compliance-samples/compliance-sample-egypt_pdpl.yaml)                   | Law 151/2020 + Decree 816/2025 (1 Nov 2025); Personal Data Protection Centre (PDPC); contextual national-ID labels (no bare 14-digit matcher); children's data as sensitive; EDM permit signal; precise geolocation (JSON lat/lng + CSV pair); 72 h Centre breach notice; grace until **31 Oct 2026**; EN + AR. |
...
| **Ethiopia (PDPP 1321/2024)** | [compliance-sample-ethiopia_pdpp.yaml](compliance-samples/compliance-sample-ethiopia_pdpp.yaml)             | Proclamation 1321/2024 (in force 24 Jul 2024); ECA; contextual Fayda FIN labels (no bare 12-digit matcher); biometric (fingerprint/face/iris); precise geolocation (JSON lat/lng + CSV pair); 72 h breach notice (revalidate); EN + draft Amharic (native review recommended). |
| **India (DPDP)**              | [compliance-sample-india_dpdp.yaml](compliance-samples/compliance-sample-india_dpdp.yaml)                   | DPDP Act 2023, DPBI; Aadhaar/PAN regex; **precise geolocation** (same geometry as VCDPA / EU GDPR); SDF / cross-border / children's data ML terms; EN; rules evolving — revalidate. |
| **Pakistan (PDPA Bill 2023)** | [compliance-sample-pakistan_pdpa.yaml](compliance-samples/compliance-sample-pakistan_pdpa.yaml)           | Personal Data Protection Bill 2023 (draft as of 2026-08 — not yet enacted); proposed NCPDP (s.35); formatted CNIC + contextual NADRA/NTN/passport labels; s.2(kk) sensitive (CNIC/passport, health, biometric, genetic, religious, political, caste/ethnicity, financial); s.14 children; precise geolocation (JSON lat/lng + CSV pair); EN + Urdu. |
| **Indonesia (PDP Law)**       | [compliance-sample-indonesia_pdp.yaml](compliance-samples/compliance-sample-indonesia_pdp.yaml)             | Law 27/2022 on Personal Data Protection; MOCLA / PDP institution; personal / specific personal data; **precise geolocation** (4 patterns); EN + Indonesian. |
| **China (PIPL)**              | [compliance-sample-china_pipl.yaml](compliance-samples/compliance-sample-china_pipl.yaml)                   | PIPL; CAC; contextual PRC resident ID + Unified Social Credit Code; **precise geolocation** (4 patterns); sensitive PI, separate consent, cross-border transfer, CII, children's PI; EN + Chinese (UTF-8) terms. |
| **Turkey (KVKK)**             | [compliance-sample-turkey_kvkk.yaml](compliance-samples/compliance-sample-turkey_kvkk.yaml)                 | Law 6698 + Amendment 7499 (2024); KVKK Board; TC Kimlik No regex; **precise geolocation**; sensitive data categories (race, health, biometric, genetic, criminal); VERBİS; 72 h breach notification; EN + TR.                                       |
| **Ukraine (PDP Law 2297-VI)** | [compliance-sample-ukraine_pdp.yaml](compliance-samples/compliance-sample-ukraine_pdp.yaml)                 | Law 2297-VI (2010, amended 2023); GDPR-convergent (EU candidacy 2022); IPN + biometric & legacy Cyrillic passport regex; **precise geolocation** (4 patterns); 98+ ML terms (EN + Ukrainian); sensitive categories (Art. 7); 72 h breach notification (Law 3454-IX 2023); ROPA, DPIA, DPO; martial law processing exceptions; cross-border transfer safeguards. |
| **New Zealand (Privacy Act)** | [compliance-sample-new_zealand_privacy.yaml](compliance-samples/compliance-sample-new_zealand_privacy.yaml) | Privacy Act 2020; OPC; IRD/NHI/Passport regex; **precise geolocation**; 13 IPPs; Notifiable Privacy Breach; sensitive categories (health, biometric, genetic, ethnicity, sexual orientation, financial); IPP 12 trans-border safeguards; EN. |
| **Russia (152-FZ)**           | [compliance-sample-russia_152_fz.yaml](compliance-samples/compliance-sample-russia_152_fz.yaml)             | Federal Law 152-FZ (266-FZ 2022), Roskomnadzor; **precise geolocation**; SNILS + heuristic INN regex; special categories (Art. 10); mandatory breach notification (72 h); localization (Art. 22.1); EN + RU terms; **revalidate often**. |
...
| **Israel (PPL + Amd. 13)**    | [compliance-sample-israel_ppl.yaml](compliance-samples/compliance-sample-israel_ppl.yaml)                   | PPL 5741-1981 + Amendment 13 (in force 14 Aug 2025); PPA; Controller/Processor; Highly Sensitive Information; contextual teudat-zehut labels (no bare 9-digit matcher); precise geolocation (JSON lat/lng + CSV pair); EN + HE. |
...
| **Mexico (LFPDPPP)**          | [compliance-sample-mexico_lfpdppp.yaml](compliance-samples/compliance-sample-mexico_lfpdppp.yaml)             | LFPDPPP, INAI; datos personales / sensibles, ARCO, aviso de privacidad; contextual CURP/RFC; **precise geolocation** (4 patterns); ES + EN. |
...
| **Ecuador (LOPDP — 2021)**    | [compliance-sample-ecuador_lopdp.yaml](compliance-samples/compliance-sample-ecuador_lopdp.yaml)             | Ley Orgánica de Protección de Datos Personales; APDP; contextual cédula/RUC/passport labels; migratory status as sensitive (Art. 4/25 — unusual among LatAm); precise geolocation (JSON lat/lng + CSV pair); children's data (Art. 25); Art. 43 breach notification (five days); GDPR-influenced; full enforcement May 2023; ES + EN. |
| **Peru (Ley 29733 — 2011)**   | [compliance-sample-peru_ley29733.yaml](compliance-samples/compliance-sample-peru_ley29733.yaml)             | Ley de Protección de Datos Personales + DS 003-2013-JUS; ANPDP; contextual DNI/RUC/passport/CE labels (no nude digit matchers); Art. 2.5 sensitive (health, ingresos económicos, convictions/union); Art. 13.6 written consent; Art. 17 confidentiality duty; precise geolocation (JSON lat/lng + CSV pair); OECD accession candidate (roadmap 2022 — not a member as of 2026-08); ES + EN. |
...
| **U.S. (FTC COPPA)**          | [compliance-sample-us_ftc_coppa.yaml](compliance-samples/compliance-sample-us_ftc_coppa.yaml)               | 15 U.S.C. §§ 6501-6506 + 16 CFR Part 312; FTC 2024 amendment; **precise geolocation**; COPPA_UNDER13_REF/VPC regex; safe harbors; ed-tech carve-out; 2024 rulemaking (biometric, targeted ads); civil penalty framing; **technical mapping** only.      |
| **U.S. (CA AB 2273)**         | [compliance-sample-us_ca_ab2273_caadca.yaml](compliance-samples/compliance-sample-us_ca_ab2273_caadca.yaml) | California Age-Appropriate Design Code **labelling**; applicability varies—counsel required; **precise geolocation** regex (same geometry as VCDPA / EU GDPR sample) for inventory signals.        |
...
| **U.S. (Virginia — VCDPA)**   | [compliance-sample-us_va_vcdpa.yaml](compliance-samples/compliance-sample-us_va_vcdpa.yaml)                 | Virginia Consumer Data Protection Act: sensitive lexicon + **precision geolocation** regexes; **no** automatic B2B/employment exemption analysis — see sample header. |
| **U.S. (California — CPRA/CCPA)** | [compliance-sample-us_ca_cpra.yaml](compliance-samples/compliance-sample-us_ca_cpra.yaml)             | CCPA as amended by CPRA: exports built-in `CCPA_SSN` + CA DL / ITIN; **precise geolocation** SPI (§ 1798.140(ae)(1)(H)); consumer-privacy ML terms; CPPA framing. |
...
Today, **tracked** product documentation is **English + pt-BR**; the **dashBOARd** web UI is largely **English** in templates. **Many** locales people ask for are **not** shipped yet—**full** dashboard and documentation surfaces in additional human languages and scripts (e.g. **es**, **fr**, **ja**, **ru**, **ar**, plus **Mandarin / Cantonese**, **Indonesian**, **Korean**, **Thai**, **Vietnamese**, **Hindi** and other **Indic** scripts, **Ethiopian / Amharic**, **broader African** contexts, **Catalan**, **Basque**, **Galician**, and more) sit on a **short- to mid-range**, **demand-driven** roadmap **together with** matching **compliance-sample** profiles where regulations warrant them. We welcome **public suggestions** (issues or discussions) on **locale or encoding gaps**; **commercial consulting** can accelerate **tailored** wording, samples, or integration hooks where the open defaults are not enough. A **public** summary of this direction (without internal planning filenames) is in the **Roadmap — internationalization and regional depth** paragraph in the repository **[README.md](../README.md)** ([pt-BR](../README.pt_BR.md)).

### How to enable and operate

1. **Choose the right sample for your region**

   See the [compliance-samples README](compliance-samples/README.md) table and the “Language and target audience” section. For example: Brazil → LGPD (PT-BR+EN); Canada → PIPEDA (EN+FR); UK → UK GDPR; EEA → EU GDPR; Benelux → Benelux sample; Japan → APPI; South Africa → POPIA; **Russia → 152-FZ** (EN+RU terms; **revalidate often**).
...
## Cyber and AI governance frameworks (evidence, not samples) {#cyber-ai-governance-evidence-not-samples}

Some buyer and auditor conversations cite **NIS2**, **DORA**, and the **EU AI Act**. Those instruments are primarily **cybersecurity**, **ICT operational resilience**, or **AI-governance** frameworks—not PII/CUI vocabulary packs. Data Boar **supports** them via **evidence and inventory** (data mapping that can feed asset registers, ICT registers, or low-risk AI positioning for a deterministic, no-LLM scanner). Do **not** expect or ship a `compliance-sample-*.yaml` for NIS2, DORA, or the EU AI Act: that would be a **category mismatch** and an overclaim risk. For **CMMC**, the fit is different: **CUI discovery is data discovery**—see [compliance-sample-us_cmmc_cui.yaml](compliance-samples/compliance-sample-us_cmmc_cui.yaml).

## Auditable and management standards (supporting role) {#auditable-and-management-standards-supporting-role}
...
**ISO 31000** is a **guideline for organizational risk management** (context, assessment, treatment, monitoring)—not a product control catalogue, and **not** summarized here. **Positioning:** your organization’s **risk management process** should **guide prioritization** of which systems to scan, which remediations to pursue first, and which regulatory profiles to enable. Data Boar **supports evidence** (discovery, mapping, metadata-only reports) and **does not replace** your risk analysis, risk acceptance, or governance decisions. **Software supply chain** and **incident-response** inventory (SBOM for dependencies and images) are documented separately as **security / release** practice—see [SECURITY.md](../SECURITY.md), [RELEASE_INTEGRITY.md](RELEASE_INTEGRITY.md), and [ADR 0003](adr/ADR-0003-sbom-roadmap-cyclonedx-then-syft.md)—and are **not** a stand-in for ISO 31000.

- **ISO/IEC 27701 (PIMS):** ISO/IEC 27701 requires PII controllers and processors to know where PII is and to document the scope of processing. Our scans and reports (findings, norm tags, Excel output) help you produce that evidence and align with the regulations you declare under ISO/IEC 27701 (e.g. LGPD, GDPR, CCPA). **Technical scope:** [COMPLIANCE_TECHNICAL_REFERENCE.md](COMPLIANCE_TECHNICAL_REFERENCE.md).
- **SOC 2:** A common pressure point for corporates; SOC 2 (Type I/II) expects documented controls over the security, availability, and confidentiality of systems that process sensitive data. We do not perform SOC 2 audits. Our **discovery and mapping** of where personal or sensitive data resides, plus metadata-only reporting, supports control design and audit preparation (e.g. evidence of data inventory and scope). Norm tags and recommendation overrides can be aligned to the trust principles and criteria you use.
- **FELCA (Lei 15.211/2025 – Estatuto Digital da Criança e do Adolescente):** In force from 17 March 2026, FELCA applies to digital platforms directed at or accessible by minors in Brazil. We do not implement age verification or platform controls. Our **minor data detection** (e.g. date-of-birth and age-related columns, “possible minor” flags) and reports help you map where data relating to minors is processed and support transparency and accountability (e.g. toward ANPD). **Complementary opt-in inventory** for LGPD Art. 14 (any entity, not only platforms): [ANPD Res. 2/2022 sample](compliance-samples/compliance-sample-brazil_anpd_res2_2022.yaml). **Technical notes:** [MINOR_DETECTION.md](MINOR_DETECTION.md).
...
**Extended sensitive categories (clinical adjacency, IP indicators, security artifacts in stored data):** The same engine can be tuned for additional lexicons and regex shapes—often with **consulting** to scope targets and reduce false positives. Positioning for buyers and DPOs (without linking to internal PMO detail): [COMPLIANCE_AND_LEGAL.md](COMPLIANCE_AND_LEGAL.md#extended-sensitive-categories-configuration-and-services).

**U.S. child and minor privacy (technical only):** Optional samples ([FTC COPPA framing](compliance-samples/compliance-sample-us_ftc_coppa.yaml), [California AB 2273](compliance-samples/compliance-sample-us_ca_ab2273_caadca.yaml), [Colorado CPA minors context](compliance-samples/compliance-sample-us_co_cpa_minors.yaml)) add **norm tags and report wording** for audit prep, DPO review, and operational scoping. They do **not** provide legal advice, verify parental consent, or prove compliance. **Disclaimers and sample intent:** [COMPLIANCE_TECHNICAL_REFERENCE.md](COMPLIANCE_TECHNICAL_REFERENCE.md) and each YAML file’s header comments under [compliance-samples/](compliance-samples/).
