...
| **Fila Dependabot (`deps`) — landar ou superseder local** | `gh pr list` **2026-08-15** · skill **dependabot-recommendations** · ADR-0069 **`rpds-py<2026`** | 🔄 Em progresso | **Actions (preferir primeiro):** **#1574** codeql-action/init 4.37.6 · **#1573** setup-python 7.0.0. **Majors uv:** **#1487** reportlab 5 · **#1485** webauthn 3 · **#1484** pyarrow 25. (**#1572** zizmor-action mergeado **2026-08-15**.) Triage com skill — **sem merge às cegas.** |
| **#1586 TCP peer pin (Mongo/Redis/SQL)** | Survey + design · slices A/B | 🔄 Em progresso | **Em `main`:** Postgres `hostaddr` (**#1589**) · Mongo `HostResolutionPin` (**#1591**) · urlsplit tests (**#1593**). **Aberto:** **#1594** fail-closed pin conflict. **Próximo:** Redis → MySQL/Oracle; mssql → [#1588](https://github.com/DataBoar/data-boar/issues/1588). Auditores: Claude Code token-aware / Codex no nó de auditoria do lab (opcional). |
