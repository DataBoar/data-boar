...
| ---- | ------ | ------ | ----- |
| **Três frentes (prioridade combinada): A sync Vault → B Maestro smoke→completão → C gate de release #406** | [OPERATOR_TODAY_MODE_2026-06-17.pt_BR.md](OPERATOR_TODAY_MODE_2026-06-17.pt_BR.md) · mapa privado `docs/private/ops/NEXT_SESSION_MAP_3_FRONTS_VAULT_MAESTRO_GATE.pt_BR.md` | ⬜ Pendente | **`[U1]`** A = CouchDB self-hosted LiveSync (warm-up isolado). **B** = primeiro completão de verdade depois das mudanças do Handler Capo (enforcement JWT) no Maestro (auditor read-only, Cursor roda `pwsh`). **C** = gate **fail-closed** — primeira rodada **sem** licença (teste #1), licença só na 2ª rodada; confirmar G1 (`py7zr`+`nfs-utils` host Void), build `boar_fast_filter` ×4, bug `$HOME=root`. Fechar quando o gate passar → release `1.7.4-rc→1.7.4`. |
| **Fila Dependabot (`deps`, não bloqueia o gate): #915 hatchling · #914 uv-minor-patch ×12 · #912 sonarqube-scan-action · #911 codeql-action** | `gh pr list` 2026-06-17 | ⬜ Pendente | Aplicar local + validar conforme `.cursor/skills/dependabot-recommendations/SKILL.md`; **sem merge às cegas** (lição do rpds-py CalVer). Trava `rpds-py<2026` + ignore no `dependabot.yml` já no lugar (ADR-0069). |
