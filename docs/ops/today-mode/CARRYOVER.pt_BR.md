...
| ---- | ------ | ------ | ----- |
| **data-boar-site [#55](https://github.com/DataBoar/data-boar-site/pull/55) — FAQ SEO** | Sessão **2026-08-07** · pós-outage Actions | ⬜ Aguarda merge | Fix guardrails: ignorar `<link rel=canonical>` (metadado) no allowlist de recursos carregados. CI **verde**. **Merge HITL** quando pronto. |
| **data-boar [#1476](https://github.com/DataBoar/data-boar/pull/1476) — ADR-0085 install priority ladder** | Sessão **2026-08-07** · `docs/adr/ADR-0085-install-priority-ladder.md` | ⬜ Aguarda merge | Fixes: fixture genesis + `INVENTORY.txt` + MD009/MD034. Checks **12/12 verdes** / `CLEAN`. **Merge HITL** (ou fechar se supersedido). |
| **Resgate de 6 ISOs raras — cópias 3-2-1** | Operador **2026-08-07** (evidência / arquivo) | ⬜ Pendente | Materializar cópias duráveis: **BICSI 1998**; **OpenSolaris 2008.05**; **TLS-6** install / source / companion / security-crypto. Inventário só em privado — sem paths/segredos no tree público. Fechar quando 3-2-1 verificado. |
| **Double-tap A.I.I.D.C.O.B.P.P. v1.5** | Operador **2026-08-07** · ADR-0062 | ⬜ Pendente | Dois pares Claude↔Cursor: papel workstation móvel + papel primary L-series (sem hostnames em docs públicos). Ritual / auditoria ping-pong — pin de modelos; sem Auto. Fechar quando ambos os pares terminarem ou adiar com data. |
