...
| **`zizmor` workflow (warn-first)** — `.github/workflows/zizmor.yml` + local **`scripts/workflow-security-lint.ps1`** / **`.sh`** (manual pre-commit hook) | Today-mode **2026-05-11** Focus A | ⬜ Pending | Commit + PR; local advisory default; enforce via **`-Enforce`**, **`DATA_BOAR_ENFORCE_ZIZMOR=true`**, or GitHub **`ZIZMOR_ENFORCE`**. |
