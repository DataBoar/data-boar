...
| ---- | ------ | ------ | ----- |
| **data-boar-site [#55](https://github.com/DataBoar/data-boar-site/pull/55) — FAQ SEO** | Session **2026-08-07** · post–Actions outage | ⬜ Await merge | Guardrails fix: ignore metadata `<link rel=canonical>` in external-resource allowlist (loaded assets only). CI **green**. **HITL merge** when ready — do not leave open after review. |
| **data-boar [#1476](https://github.com/DataBoar/data-boar/pull/1476) — ADR-0085 install priority ladder** | Session **2026-08-07** · `docs/adr/ADR-0085-install-priority-ladder.md` | ⬜ Await merge | Fixes applied: genesis fixture + `INVENTORY.txt` + MD009/MD034. Checks **12/12 green** / `CLEAN`. **HITL merge** (or close if superseded). |
| **Rescue 6 rare ISOs — 3-2-1 copies** | Operator **2026-08-07** (evidence / archive) | ⬜ Pending | Materialize durable copies: **BICSI 1998**; **OpenSolaris 2008.05**; **TLS-6** install / source / companion / security-crypto. Private inventory only — no secrets/paths in public tree. Close when 3-2-1 verified. |
| **Double-tap A.I.I.D.C.O.B.P.P. v1.5** | Operator **2026-08-07** · ADR-0062 | ⬜ Pending | Two Claude↔Cursor pairs: mobile workstation role + L-series primary role (no hostnames in public docs). Ritual / ping-pong audit — pin models; no Auto. Close when both pairs finish or defer with date. |
| **Bestiary doctrine sidequest (#994) — 10 repos, vault SOUL → DOUTRINA + ADRs** | Sessions **2026-07-01**–**03** · `docs/private/commercial/bestiais/BEASTIE_INVENTORY_HONEST_2026-07-01.pt_BR.md` · vault `bestiais/` | 🔄 In progress | **Done:** honest inventory + roster; Homing Robin birth-triplet **pushed**; local drafts **Faithful Ferret / Sage Remora / Stealthy Stoat** (2026-07-02). **Next:** finish 7 repos (Quati, Tortoise, Mimic, Llama, Otter, Squirrel + Robin #2 enrich) — **one PR/repo**. **Maestro** moved to private **PR #9** phase-1 migrate (see row below). |
| **Maestro private repo — phase-1 migration** | `FabioLeitao/maestro` · `docs/ops/MAESTRO_SPINOUT.md` | 🔄 In progress | **PR #9** `feat/phase1-migrate-from-data-boar` (ADR-0001) open · **PR #7** docs skeleton. Canonical handlers still **`data-boar/scripts/maestro*`** until merge. Public **#786** shipped via **#787**. |
