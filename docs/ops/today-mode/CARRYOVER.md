...
| **Dependabot queue (`deps`) — land or supersede locally** | `gh pr list` **2026-08-15** · skill **dependabot-recommendations** · ADR-0069 **`rpds-py<2026`** | 🔄 In progress | **Actions (prefer first):** **#1574** codeql-action/init 4.37.6 · **#1573** setup-python 7.0.0. **Major uv:** **#1487** reportlab 5 · **#1485** webauthn 3 · **#1484** pyarrow 25. (**#1572** zizmor-action merged **2026-08-15**.) Triage with skill — **no blind merge.** |
| **#1586 TCP peer pin (Mongo/Redis/SQL)** | Survey + design · slices A/B | 🔄 In progress | **On `main`:** Postgres `hostaddr` (**#1589**) · Mongo `HostResolutionPin` (**#1591**) · urlsplit tests (**#1593**). **Open:** **#1594** fail-closed pin conflict. **Next:** Redis → MySQL/Oracle; mssql → [#1588](https://github.com/DataBoar/data-boar/issues/1588). Auditors: Claude Code token-aware / optional Codex on the lab audit node. |
