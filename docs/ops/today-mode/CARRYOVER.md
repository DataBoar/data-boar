...
| ---- | ------ | ------ | ----- |
| **Three fronts (agreed priority): A Vault sync → B Maestro smoke→completão → C release gate #406** | [OPERATOR_TODAY_MODE_2026-06-17.md](OPERATOR_TODAY_MODE_2026-06-17.md) · private map `docs/private/ops/NEXT_SESSION_MAP_3_FRONTS_VAULT_MAESTRO_GATE.pt_BR.md` | ⬜ Pending | **`[U1]`** A = self-hosted CouchDB LiveSync (isolated warm-up). **B** = first real completão after Maestro Capo-handler JWT-enforcement changes (auditor read-only, Cursor runs `pwsh`). **C** = gate **fail-closed** — first run **without** license (test #1), license only 2nd round; confirm G1 (`py7zr`+`nfs-utils` Void host), `boar_fast_filter` build ×4, `$HOME=root` bug. Close when gate passes → `1.7.4-rc→1.7.4` release. |
| **Dependabot queue (`deps`, not blocking gate): #915 hatchling · #914 uv-minor-patch ×12 · #912 sonarqube-scan-action · #911 codeql-action** | `gh pr list` 2026-06-17 | ⬜ Pending | Apply locally + validate per `.cursor/skills/dependabot-recommendations/SKILL.md`; **no blind-merge** (rpds-py CalVer lesson). `rpds-py<2026` cap + `dependabot.yml` ignore already in place (ADR-0069). |
