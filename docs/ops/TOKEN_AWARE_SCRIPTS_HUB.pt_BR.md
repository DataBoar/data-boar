...
| `private-git-sync.ps1` / `private-git-sync.sh` | Repo privado empilhado (`-Push` / `--push`: bare lab **mini-bt, latitude, t14, alpine-emachines** -- **pi3b excluido**; bare VC em **`Z:`**/**`Y:`** se montado; mirror pCloud) | **`docs/ops/PRIVATE_LOCAL_VERSIONING.md`**, **`PRIVATE_STACK_SYNC_RITUAL.md`**, sessao **`private-stack-sync`**, **`PRIVATE_OPERATOR_NOTES.md`**, **ADR 0040**, **`operator-evidence-backup-no-rhetorical-asks.mdc`** |
