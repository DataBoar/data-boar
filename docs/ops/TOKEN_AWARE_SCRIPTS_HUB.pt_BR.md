...
### 1d. Bootstrap Gitleaks / OSV (tier de segurança do check-all, #1933)

| Script | Função | Ligado a |
| ------ | ------ | -------- |
| `tool-pins.sh` | Versão + SHA256 do **binário** para gitleaks **8.30.1** e osv-scanner **2.6.0**; `DB_GITLEAKS_POLICY_CONFIG=security/gitleaks.toml` | Sourced pelos blocos `run:` do CI e pelo bootstrap |
| `db-tool-bootstrap.sh` | Baixa CLIs pinadas em **`scripts/.cache/`** (gitignored) | `run-gitleaks-strict.sh`, `run-osv-scanner.sh` |
| `run-gitleaks-strict.sh` | `rm -f .gitleaks.toml .gitleaksignore` e depois `gitleaks git . --ignore-gitleaks-allow` | Tier de segurança **padrão** do **`check-all`** |
| `run-osv-scanner.sh` | `osv-scanner scan source -r . --config=security/osv-scanner.toml` | Só **`check-all --enforced`** |
| `check-all-security-scans.sh` / `.ps1` | Bandit + Zizmor + Gitleaks; **`--enforced`** soma OSV + Semgrep | **`check-all.sh`** / **`check-all.ps1`**; [TESTING.pt_BR.md](../TESTING.pt_BR.md) *Gitleaks + OSV* |
...
| `databoar-env-load.sh` / `databoar-env-load.ps1` | Dot-source: carrega `~/.config/databoar/*.env` no shell (ponte opcional; preferir vault → env) | **`OPERATOR_CREDENTIALS_FROM_ENV.pt_BR.md`**, **`OPERATOR_SECRETS_BITWARDEN.pt_BR.md`**, **`PLAN_SECRETS_VAULT.md`** (Fase B) |
...
| `run-benchmark.ps1` + `benchmark_sqlite_diff.py` | *Harness* A/B opcional: duas execuções cronometradas de **`lab-completao-orchestrate.ps1`** (**`-LabGitRef`**, ex.: **`v1.7.3`** vs **`origin/main`**), cópia opcional de SQLite (**`-SqlitePath`**), **`data-boar-report`** opcional após a 2ª rodada, gera **`bench/COMPARISON.log`**; o Python compara dois SQLite quando existem **`bench/v1.7.3/results.db`** e **`bench/v1.7.4/results.db`** | **`LAB_COMPLETAO_RUNBOOK.pt_BR.md`** (*Target git ref*); pasta **`bench/`** no `.gitignore` |
| `benchmark-ab.ps1` | A/B com **Git** no clone do dev: bloqueio de árvore suja (**`-AutoStash`** / **`-AllowDirty`**), **`git checkout`** na tag legada e **`lab-completao-orchestrate.ps1`**, volta ao branch/HEAD, segunda passagem do completão, copia/move logs (SQLite opcional / **`data-boar-report`**) em **`benchmark_runs/`**, grava **`benchmark_runs/times.txt`** com totais do **`Measure-Command`** | **`LAB_COMPLETAO_RUNBOOK.pt_BR.md`** (*Target git ref*; *blast radius* do PC dev: só checkout normal); pasta **`benchmark_runs/`** no `.gitignore` |
...
| `maestro-deep-rc-monitor-collect.ps1` | Sessão de lab em um comando: resolve **[DataBoar/maestro](https://github.com/DataBoar/maestro)** via **`Resolve-MaestroRoot.ps1`** (`MAESTRO_ROOT` ou sibling `../maestro`), depois **`core/Maestro.ps1 -Deep`**, loop opcional de **`Get-LabStatus`** (padrão **15** x **20s**), **`Maestro.ps1 -Collect`**; **`-SkipMonitor`**, **`-SkipCollect`**, **`-ClearHostEachLoop`**; **`Ref`** repassado; código de saída = maior não zero dos filhos (o preflight de imagem/tar no Maestro ainda pede **Docker**/**Podman** local no Windows quando esse caminho roda) | **`scripts/Resolve-MaestroRoot.ps1`**, DataBoar/maestro **`core/Maestro.ps1`**, privado **`docs/private/homelab/data/inventory.json`**, **`LAB_COMPLETAO_RUNBOOK.pt_BR.md`**, **`tests/config/benchmark-rc-v3.yaml`** (v2 mantido), **`scripts/benchmark_rc_sentinel_check.py`**, **`tests/test_maestro_scripts.py`** |
| `benchmark_rc_sentinel_check.py` | Gate pós-scan para RC v3 do Maestro: limites no SQLite (`min_count` / `max_count`, substrings proibidas) + YAML negativo SSRF/auth (`#832`, `#1977`, `#1985`); chamado por **`lab-completao-host-smoke.sh`** salvo **`LAB_COMPLETAO_SKIP_RC_SENTINEL=1`** | **`tests/config/benchmark-rc-v3.sentinel.yaml`**, **`tests/config/benchmark-rc-ssrf-negative.yaml`**, **`tests/test_benchmark_rc_sentinel_check.py`**, **`docs/plans/PLAN_RC_SENTINEL_UPPER_BOUND_SCHEMA.md`** |
...
| `growatt.ps1`, `enel.ps1`, `energy-report.ps1` | Solar / rede / correlação | SKILL **`homelab-lab-op-data`**, **`solar-snap`**, **`enel-check`**, **`energy-report`**, **`LAB-ROUTER-01-scan`** |
| `growatt-session-collect.ps1`, `enel-session-collect.ps1` | Janela de sessão | SKILL **`session-aware-collect`**, **`session-collect`** |
| `LAB-ROUTER-01.ps1`, `snmp-LAB-ROUTER-01-lab-probe*.ps1`, `LAB-ROUTER-01-api-*.ps1` | UniFi / LAB-ROUTER-01 | SKILL **`homelab-lab-op-data`**, **`LAB-ROUTER-01-scan`** |
...
| `private-git-sync.ps1` / `private-git-sync.sh` | Repo privado empilhado (`-Push` / `--push`: **mirrors bare lab** conforme manifesto nos nos x86 principais; **no ARM de baixa potencia** fica de fora do push bare — alvo de validacao, nao espelho; lista canonica no script + manifesto privado; bare VC em **`Z:`**/**`Y:`** se montado; mirror pCloud) | **`docs/ops/PRIVATE_LOCAL_VERSIONING.md`**, **`PRIVATE_STACK_SYNC_RITUAL.md`**, sessao **`private-stack-sync`**, **`PRIVATE_OPERATOR_NOTES.md`**, **ADR 0040**, **`operator-evidence-backup-no-rhetorical-asks.mdc`** |
...
| `emit-provenance.ps1` | Registro local de provenance **não assinado** (não é SLSA) | **`SECURITY.md`**, **`#1950`**, workflow **`SBOM`** |
