...
| Job behaviour         | With the variable **not** `false`, a zizmor finding **fails the job** (`ENFORCE_ZIZMOR`). That job is **not** in `required_status_checks`. On `pull_request`, SARIF uploads to **`refs/pull/<N>/head`** (not merge) so ruleset **21118976** can match the PR head SHA — see comments in [`.github/workflows/zizmor.yml`](../../.github/workflows/zizmor.yml). |
