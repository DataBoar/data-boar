...
| Job behaviour         | With the variable **not** `false`, a zizmor finding **fails the job** (`ENFORCE_ZIZMOR`). That job is **not** in `required_status_checks`. On `pull_request`, `zizmor-action` must keep **`advanced-security: true`** (otherwise `output-file` is empty) and a follow-up **`upload-sarif`** step must target **`refs/pull/<N>/head`** — ruleset **21118976** matches head SHA only. |
