...
| -------------                       | --------                                                                                 | -------------------                                                                                                           | ----------------               | -----                                                                                                                                                                                                 |
| **Red Hat Enterprise Linux (RHEL)** | **Enterprise** deployments often use RHEL; **AlmaLinux** / **Rocky** are RHEL-compatible | **RHEL 9+** has Python 3.12 in **AppStream**; **RHEL 8** may need **EPEL** or **Python 3.12** from **SCL** / **rh-python312** | **`dnf`** (or `yum` on RHEL 7) | **System deps:** `dnf install python3.12 python3.12-devel gcc openssl-devel libffi-devel postgresql-devel unixODBC-devel` (adjust for RHEL 8 vs 9). **Onboarding gap (pipx):** some RHEL9-family hosts still resolve default `python3=3.9`, which can fail with `Requires-Python >=3.12` / `No matching distribution`. Use `pipx install --python python3.12 data-boar`. |
