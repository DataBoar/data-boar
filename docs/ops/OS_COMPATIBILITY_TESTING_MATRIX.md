...
| **Void Linux**   | **musl** option; **xbps** package manager; **lightweight**                           | **Void** has Python 3.12+; **musl** vs **glibc** can affect **native wheels** | **`xbps-install`** | **Proven split (1.7.4.post3 retest):** **Void-glibc** passes in default `pipx install data-boar` (PyPI publishes `cp314` wheel). **Void-musl (py3.14.6):** with toolchain (`xbps-install -S python3-devel gcc gcc-fortran openblas-devel`), `pipx install` succeeds and `--demo` returns **26 findings** (runtime parity with glibc). Without toolchain, install currently fails only on `scikit-learn==1.9.0` source build (while `numpy`/`pandas` wheels are available, `scipy` (direct dependency) also has a musllinux wheel on PyPI, and `odfpy` is pure Python). Remaining moat for pure pipx path: publish a `cp314 musllinux` wheel for scikit-learn (tracking [#1182](https://github.com/DataBoar/data-boar/issues/1182)). |
...
| **Void-musl** | Parity with toolchain | Install build prerequisites (`python3-devel gcc gcc-fortran openblas-devel`) before `pipx install`; gap for toolchain-free path is currently one wheel (`scikit-learn` `cp314 musllinux`, [#1182](https://github.com/DataBoar/data-boar/issues/1182)) |
