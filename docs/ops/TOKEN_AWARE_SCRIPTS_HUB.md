...
| `private-git-sync.ps1` / `private-git-sync.sh` | Nested private repo (`-Push` / `--push`: lab bare mirrors **mini-bt, latitude, t14, alpine-emachines** only -- **pi3b excluded**; VC bare on `Z:`/`Y:` when mounted; pCloud tree mirror) | **`docs/ops/PRIVATE_LOCAL_VERSIONING.md`**, **`PRIVATE_STACK_SYNC_RITUAL.md`**, session **`private-stack-sync`**, **`PRIVATE_OPERATOR_NOTES.md`**, **ADR 0040**, **`operator-evidence-backup-no-rhetorical-asks.mdc`** |
