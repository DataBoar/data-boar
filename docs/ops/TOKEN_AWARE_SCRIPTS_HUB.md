...
| `generate_f1_validation_fixtures.py` | Labeled F1 fixtures + `ground_truth.yaml` (measure vs calibrate anti-leakage) | [VALIDATION.md](../VALIDATION.md), #835, [PLAN_SYNTHETIC_DATA_AND_CONFIDENCE_VALIDATION.md](../plans/PLAN_SYNTHETIC_DATA_AND_CONFIDENCE_VALIDATION.md) |
| `validate_detection_f1.py` | Precision/recall/F1 vs detector tuple (default `--split measure`) | Same; `uv run python scripts/validate_detection_f1.py` |

### 1b. GitHub Actions YAML contract (pytest — not under `scripts/`)
...
| `video-frame-samples.ps1` | Sparse **PNG** frames from video (**`ffmpeg`** / **`ffprobe`**; operator **timestamps**) | **`docs/ops/VIDEO_FRAME_EXTRACT_FOR_AGENT.md`**, **`.cursor/skills/video-frames-for-agent/SKILL.md`**, **`.cursor/rules/repo-scripts-wrapper-ritual.mdc`** |

### 1c. Still images + audio + video tags (operator inspect)

| Script | Role | Wired to |
| ------ | ---- | -------- |
| `image-inspect.ps1` | **uv** wrapper: **`image_inspect.py`** — EXIF / mutagen / **ffprobe** + optional **Tesseract** (same stack as **`rich_media_sample`**) | **`docs/ops/IMAGE_EXIF_OCR_FOR_AGENT.md`**, **`.cursor/skills/image-inspect-for-agent/SKILL.md`** |
| `image_inspect.py` | One-file CLI over **`build_rich_media_text_sample`** | Same |
| `icloud-photos-fetch-range.ps1` | Stage iCloud / Apple Photos files into **`docs/private/icloud_temp/`** (then inspect) | Private **`ICLOUD_PHOTOS_SYNC_GUIDE.md`** when present; **`session-collect`** |
...
| `gate-trailer-attest.ps1` / `gate-trailer-attest.sh` | Verify/sign decorative **file**-namespace SSH signature on `Gate-Change-Approved-By:` / `Gate-Close-Approved-By:` trailer lines (`gate_trailer_attest.py`) | **ADR-0071**, **ADR-0056** (`docs/adr/allowed_signers`); complements **`gate_change_tripwire.py`** (regex only) + **`git verify-commit`** (`git` namespace) |
...
| `primary-linux-agent-sessions.sh` | **Linux primary only:** tmux **`databoar-cursor`** (`agent worker start`) + **`databoar-claude`** (`claude` → `/rc`); optional **systemd user** unit | **`PRIMARY_LINUX_WORKSTATION_PROTECTION.md`**, **`agent-roles-executor-vs-auditor.mdc`** (Cursor executor × Claude auditor) |
| `databoar-env-load.sh` / `databoar-env-load.ps1` | Dot-source: load `~/.config/databoar/*.env` into the shell (optional bridge; vault → env preferred) | **`OPERATOR_CREDENTIALS_FROM_ENV.md`**, **`OPERATOR_SECRETS_BITWARDEN.md`**, **`PLAN_SECRETS_VAULT.md`** (Phase B) |
...
| `run-benchmark.ps1` + `benchmark_sqlite_diff.py` | Optional A/B harness: two timed **`lab-completao-orchestrate.ps1`** runs (**`-LabGitRef`**, e.g. **`v1.7.3`** vs **`origin/main`**), optional **`-SqlitePath`** copies, optional **`data-boar-report`** after round 2, writes **`bench/COMPARISON.log`**; Python helper diffs two SQLite files when both **`bench/v1.7.3/results.db`** and **`bench/v1.7.4/results.db`** exist | **`LAB_COMPLETAO_RUNBOOK.md`** (*Target git ref*); **`bench/`** is gitignored |
| `benchmark-ab.ps1` | **Git** A/B on the dev clone: guard dirty tree (**`-AutoStash`** / **`-AllowDirty`**), **`git checkout`** legacy tag then **`lab-completao-orchestrate.ps1`**, restore branch/HEAD, second completão pass, copy/move logs (and optional SQLite / **`data-boar-report`**) under **`benchmark_runs/`**, append **`benchmark_runs/times.txt`** with **`Measure-Command`** totals | **`LAB_COMPLETAO_RUNBOOK.md`** (*Target git ref*, *Primary Windows dev PC* blast radius: normal checkout only); **`benchmark_runs/`** is gitignored |
...
| `maestro-deep-rc-monitor-collect.ps1` | One-shot lab session: resolves **[DataBoar/maestro](https://github.com/DataBoar/maestro)** via **`Resolve-MaestroRoot.ps1`** (`MAESTRO_ROOT` or sibling `../maestro`), then **`core/Maestro.ps1 -Deep`**, optional **`Get-LabStatus`** loop (defaults **15** x **20s**), **`Maestro.ps1 -Collect`**; **`-SkipMonitor`**, **`-SkipCollect`**, **`-ClearHostEachLoop`**; **`Ref`** passed through; exit code = max non-zero child (Docker preflight in Maestro still needs a local engine when the tarball build path runs) | **`scripts/Resolve-MaestroRoot.ps1`**, DataBoar/maestro **`core/Maestro.ps1`**, private **`docs/private/homelab/data/inventory.json`**, **`LAB_COMPLETAO_RUNBOOK.md`**, **`tests/config/benchmark-rc-v3.yaml`** (v2 retained), **`scripts/benchmark_rc_sentinel_check.py`**, **`tests/test_maestro_scripts.py`** |
| `benchmark_rc_sentinel_check.py` | Post-scan gate for Maestro RC v3: SQLite bounds (`min_count` / `max_count`, forbidden substrings) + SSRF/auth negative YAML (`#832`, `#1977`, `#1985`); wired from **`lab-completao-host-smoke.sh`** unless **`LAB_COMPLETAO_SKIP_RC_SENTINEL=1`** | **`tests/config/benchmark-rc-v3.sentinel.yaml`**, **`tests/config/benchmark-rc-ssrf-negative.yaml`**, **`tests/test_benchmark_rc_sentinel_check.py`**, **`docs/plans/PLAN_RC_SENTINEL_UPPER_BOUND_SCHEMA.md`** |
...
| `lab-node-01-ansible-baseline.ps1` | Ansible baseline | Lab host provisioning (private details stay private) |
| `lab-node-01-ansible-labop-podman-apply.sh` | Podman Ansible (sudoers-fixed args) | Invoked via `sudo -n` + narrow `LABOP_ANSIBLE_PODMAN`; see **LAB_OP_PRIVILEGED_COLLECTION.md** |
