...
2. Na **raiz do repo do produto**, corre **`scripts/private-git-sync.ps1`** (Windows) ou **`scripts/private-git-sync.sh --push`** (Linux/macOS); opcional **`-Push`** / **`--push`** se a política enviar o repo empilhado para remotes **fora do GitHub**. Os **bare mirrors lab** seguem a lista do manifesto no script (nos x86 principais); o **no ARM de baixa potencia** fica de fora do push bare — alvo de validacao, nao espelho.
