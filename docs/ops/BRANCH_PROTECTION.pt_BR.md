...
| Quando roda                     | `pull_request` / `push` em `main` quando **`.github/workflows/**`** muda; agenda semanal; `workflow_dispatch`                                                                                                                  |
| Comportamento do job            | Com a variável **diferente** de `false`, um achado do zizmor **falha o job** (`ENFORCE_ZIZMOR`). Esse job **não** está em `required_status_checks`: é sinal fail-closed no caminho de **workflows**, não gate global de merge. |
