...
| Comportamento do job            | Com a variável **diferente** de `false`, um achado do zizmor **falha o job** (`ENFORCE_ZIZMOR`). Esse job **não** está em `required_status_checks`. Em `pull_request`, o SARIF sobe em **`refs/pull/<N>/head`** (não merge) para o ruleset **21118976** casar com o SHA do head — veja comentários em [`.github/workflows/zizmor.yml`](../../.github/workflows/zizmor.yml). |
