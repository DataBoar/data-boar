...
| Comportamento do job            | Com a variável **diferente** de `false`, um achado do zizmor **falha o job** (`ENFORCE_ZIZMOR`). Esse job **não** está em `required_status_checks`. Em `pull_request`, manter **`advanced-security: true`** no `zizmor-action` (senão `output-file` fica vazio) e um **`upload-sarif`** explícito em **`refs/pull/<N>/head`** — ruleset **21118976** casa só com o SHA do head. |
