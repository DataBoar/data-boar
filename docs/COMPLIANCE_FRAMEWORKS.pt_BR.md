...
<a id="detecção-determinística-vs-hype-de-llm-generativo"></a>

### Detecção determinística vs hype de LLM generativo (postura do produto)

**Evidência descoberta vs narrativa de “IA”.** O motor produz **achados repetíveis** a partir de **regex**, **padrões nomeados** e **ML/DL supervisionado** com **seus** termos de treino — além de sinais estruturados como **agregação de quasi-identificadores**, colunas ligadas a **menores** e **dicas de jurisdição / tensão** (sempre **heurística**, nunca “lei aplicável” no software). Esse contrato difere de **LLMs generativos**, que otimizam texto fluente e podem variar entre execuções. Arquitetura e expectativa de auditoria: [TECH_GUIDE.pt_BR.md](TECH_GUIDE.pt_BR.md#detection-stack-vs-generative-llms); vocabulário (**LLM**, **pilha de detecção determinística**, **Google Gemini** como ferramenta de manutenção): [GLOSSARY.pt_BR.md](GLOSSARY.pt_BR.md) §8–9 e [docs/ops/LLM_AGENT_EDITING_CAUTION.pt_BR.md](ops/LLM_AGENT_EDITING_CAUTION.pt_BR.md). Linha do tempo **neutra face a fornecedores** (anos 1950 → **invernos da IA** → **sistemas especialistas** → ML estatístico → **DL** → **LLMs**): [AI_EVOLUTION_PRIMER.pt_BR.md](primers/AI_EVOLUTION_PRIMER.pt_BR.md) ([EN](primers/AI_EVOLUTION_PRIMER.md)).
...
**Amostras de conformidade (um perfil por regulamento):** Amostras de config para **UK GDPR**, **EU GDPR**, **UE NIS2**, **Benelux**, **PIPEDA**, **POPIA**, **APPI**, **PCI-DSS**, **Rússia (Lei federal 152-FZ)** e outras regiões estão em [compliance-samples/](compliance-samples/). Cada amostra é um único arquivo YAML com padrões regex, termos ML e recommendation overrides para esse framework. A **NIS2** é uma diretiva de **cibersegurança** (não é lei de dado pessoal); ela **complementa o GDPR** porque incidentes NIS2 frequentemente disparam notificação de violação GDPR em paralelo — mescle [compliance-sample-eu_nis2.yaml](compliance-samples/compliance-sample-eu_nis2.yaml) com a amostra EU GDPR. Veja a seção [Amostras de conformidade](#amostras-de-conformidade) abaixo para a lista completa, o que vai em cada arquivo e como usar. Para **idioma e público-alvo** (ex.: PIPEDA → EN + FR no Canadá; **152-FZ** → termos **EN + RU** na amostra russa), veja o [README de compliance-samples](compliance-samples/README.pt_BR.md).
...
| --------------------            | -------                                                                                                     | ----------                                                                                                                          |
| **LGPD (Brasil)**               | [compliance-sample-lgpd.yaml](compliance-samples/compliance-sample-lgpd.yaml)                               | Termos PT-BR + EN; regex RG/CEP; regex de **geolocalização precisa** (mesma geometria que as amostras VCDPA / RGPD UE); implantações brasileiras.                                                                          |
...
| **Brasil (FELCA — Lei 15.211/2025)** | [compliance-sample-brazil_felca.yaml](compliance-samples/compliance-sample-brazil_felca.yaml) | Estatuto Digital da Criança e do Adolescente (plataformas digitais e menores); **`norm_tag`** e termos ML de inventário; **mescle** com [amostra LGPD](compliance-samples/compliance-sample-lgpd.yaml); **não** é verificação de idade (ver tópico FELCA em [Normas auditáveis e de gestão](#normas-auditaveis-e-de-gestao-papel-de-suporte) e [MINOR_DETECTION.pt_BR.md](MINOR_DETECTION.pt_BR.md)). |
| **Brasil (ANPD Res. 2/2022 — Dados de menores)** | [compliance-sample-brazil_anpd_res2_2022.yaml](compliance-samples/compliance-sample-brazil_anpd_res2_2022.yaml) | Resolução CD/ANPD 2/2022 (LGPD Art. 14); consentimento parental verificável, específico e revogável; **proibição** de perfilamento comportamental (norm_tag: `perfilamento proibido — menores`); restrições a dado educacional; vigência 2023; qualquer entidade (não só plataformas); complementa FELCA + LGPD; PT-BR + EN. |
| **Brasil (EAD / educação — LGPD Art. 14 + ECA)** | [compliance-sample-br_ead_lgpd_art14.yaml](compliance-samples/compliance-sample-br_ead_lgpd_art14.yaml) | Inventário de EAD e escola/LMS: RA / matrícula, e-mail `.edu.br`, CPF **contextual** em campo educacional (não é matcher de CPF isolado); ECA (Lei 8.069/1990) + LGPD Art. 14; mescle com LGPD + ANPD Res. 2/2022 + FELCA; **não** é verificação de idade nem certificação MEC/ANPD; PT-BR + EN. |
| **Brasil (setor saúde — LGPD Art. 11)** | [compliance-sample-brazil_saude.yaml](compliance-samples/compliance-sample-brazil_saude.yaml) | Identificadores de saúde (CRM/CRF/CRO/COREN, CNS, RMS ANVISA, ANS) e léxico de dado de saúde; **mescle** com [amostra LGPD](compliance-samples/compliance-sample-lgpd.yaml); regex heurística — **sem** validação de dígito de CRM. |
| **Brasil (BACEN Res. 4893/2021 — Cibersegurança)** | [compliance-sample-brazil_bacen4893.yaml](compliance-samples/compliance-sample-brazil_bacen4893.yaml) | Política de segurança cibernética BACEN para bancos, fintechs e instituições de pagamento; regex de chave Pix, conta, token de cartão, NSU; notificação de incidente em 4 dias úteis ao BACEN (Art. 11); terceirização de TI (Art. 16); controles criptográficos; vigência fev 2022; mescle com amostras LGPD + PCI-DSS; PT-BR + EN. |
| **UK GDPR**                     | [compliance-sample-uk_gdpr.yaml](compliance-samples/compliance-sample-uk_gdpr.yaml)                         | Pós-Brexit, ICO; norm_tag e recommendation overrides; regex de **geolocalização precisa** (mesma geometria VCDPA / RGPD UE).                                                                               |
...
| **UE NIS2 (Diretiva 2022/2555)** | [compliance-sample-eu_nis2.yaml](compliance-samples/compliance-sample-eu_nis2.yaml)                         | Segurança de redes e informação; entidades essenciais/importantes; medidas de cibersegurança (Art. 21); notificação de incidente 24h+72h ao CSIRT/autoridade competente (Art. 23); cadeia de suprimentos; divulgação de vulnerabilidade (Art. 12); detecção de credenciais/chaves/segredos MFA; mescle com a amostra EU GDPR; **não** é lei de dado pessoal (complementa o GDPR quando o incidente também dispara notificação de violação); prazo de transposição 17/10/2024 (aplicação depende da transposição nacional); EN. |
| **Benelux (BE, NL, LU)**        | [compliance-sample-benelux.yaml](compliance-samples/compliance-sample-benelux.yaml)                         | Base EU GDPR + IDs nacionais (BSN, NISS, LU); overrides DPA nacionais; EN + NL/FR/DE (terceira língua oficial da Bélgica / Ostbelgien). |
| **Alemanha (BDSG)**            | [compliance-sample-germany_bdsg.yaml](compliance-samples/compliance-sample-germany_bdsg.yaml)               | BDSG como suplemento nacional do GDPR; rótulos contextuais Steuer-ID / empregado / biometria / scoring (sem matcher nu de 11 dígitos); §26 dados de empregados **contestado** (TJUE C-34/21); §31 scoring; 16 DPAs estaduais + BfDI; mesclar com EU GDPR; EN + DE. |
| **HIPAA / PHI (EUA Saúde)**    | [compliance-sample-us_hipaa_phi.yaml](compliance-samples/compliance-sample-us_hipaa_phi.yaml)               | Privacy Rule (45 CFR Parte 164) + Security Rule + Breach Notification Rule + HITECH 2009; HHS OCR; NPI + DEA + MRN + ICD-10 regex; **geolocalização precisa** (dado geográfico < estado = identificador PHI Safe Harbor #3); 95+ termos ML; entidade coberta / BAA / mínimo necessário / ePHI; notas de psicoterapia + 42 CFR Parte 2 + GINA; notificação em 60 dias; desidentificação; EN. |
| **FERPA (EUA Educação)**      | [compliance-sample-us_ferpa.yaml](compliance-samples/compliance-sample-us_ferpa.yaml)                         | 20 U.S.C. § 1232g; FPCO; sinais de inventário de registros escolares; regex SSN + ID de aluno contextual + DOB; **geolocalização precisa** (4 padrões); LMS/SIS, histórico escolar, informação de diretório; complementar à COPPA para menores online; EN. |
| **CMMC / CUI (EUA Defesa)**   | [compliance-sample-us_cmmc_cui.yaml](compliance-samples/compliance-sample-us_cmmc_cui.yaml)                 | Marcações/banners CUI, rótulos de categoria, controles de disseminação (NOFORN, REL TO, …); ITAR/EAR/USML/CCL como **sinais de inventário/triagem**; ~60 termos ML; **apoia evidência CMMC / descoberta de CUI** — **não** certifica CMMC, **não** avalia nível CMMC nem determina aplicabilidade ITAR/EAR; EN. |
| **PIPEDA (Canadá)**             | [compliance-sample-pipeda.yaml](compliance-samples/compliance-sample-pipeda.yaml)                           | Informação pessoal, consentimento; identificadores canadenses (ex.: SIN); regex de **geolocalização precisa** (mesma geometria VCDPA / RGPD UE); EN + FR.                                                  |
| **Québec (Lei 25)**            | [compliance-sample-canada_qc_law25.yaml](compliance-samples/compliance-sample-canada_qc_law25.yaml)         | Lei sobre proteção de dados no setor privado (QC, modernizada 2023); CAI; complementa PIPEDA federal no setor privado de QC; **geolocalização precisa** (4 padrões); EN + FR. |
| **POPIA (África do Sul)**       | [compliance-sample-popia.yaml](compliance-samples/compliance-sample-popia.yaml)                             | POPIA Lei 4/2013; Regulador de Informação; regex ID sul-africano; **geolocalização precisa** (mesma geometria VCDPA / EU GDPR); informação pessoal especial (s.26), dados de crianças (s.34), transferências transfronteiriças (s.72); EN. |
| **APPI (Japão)**                | [compliance-sample-appi.yaml](compliance-samples/compliance-sample-appi.yaml)                               | Informação pessoal e sensível (要配慮個人情報); My Number; regex de **geolocalização precisa** (位置情報; mesma geometria VCDPA / EU GDPR); termos EN + japonês; alinhamento PPC; revisão 2022.                    |
...
| **Filipinas (DPA)**             | [compliance-sample-philippines_dpa.yaml](compliance-samples/compliance-sample-philippines_dpa.yaml)         | RA 10173 + NPC IRR 2016; NPC; regex SSS/GSIS/TIN/PhilSys; **geolocalização precisa**; categorias SPI (raça, saúde, genético, criminal, IDs governamentais); notificação de violação em 72 h; EN. |
| **Austrália (Privacy Act)**     | [compliance-sample-australia_privacy.yaml](compliance-samples/compliance-sample-australia_privacy.yaml)     | Privacy Act 1988 (Cth; Emenda 2024), OAIC, APPs; regex TFN/Medicare/ABN/Passaporte; **geolocalização precisa** (mesma geometria VCDPA / EU GDPR); esquema NDB; informação sensível (s.6); EN. |
| **Singapura (PDPA)**            | [compliance-sample-singapore_pdpa.yaml](compliance-samples/compliance-sample-singapore_pdpa.yaml)           | PDPA 2012 (alterada 2020/2021), PDPC; regex NRIC/FIN/UEN; **geolocalização precisa** (mesma geometria VCDPA / EU GDPR); DNC, notificação obrigatória de violação; EN. |
| **Malásia (PDPA + Emenda 2024)** | [compliance-sample-malaysia_pdpa.yaml](compliance-samples/compliance-sample-malaysia_pdpa.yaml)             | PDPA 2010 + Lei A1727 (fases 1 jan / 1 abr / 1 jun 2025, todas em vigor); Comissário; Data Controller/Processor; DPO; MyKad formatado + rótulos contextuais (sem matcher nu de 12 dígitos); biométrico como sensível; geolocalização precisa (JSON lat/lng + par CSV); EN. |
| **Tailândia (PDPA)**           | [compliance-sample-thailand_pdpa.yaml](compliance-samples/compliance-sample-thailand_pdpa.yaml)              | Lei de Proteção de Dados Pessoais B.E. 2562 (2019), PDPC; dado pessoal / sensível; **geolocalização precisa** (4 padrões); EN + tailandês. |
| **Vietnã (PDPD)**              | [compliance-sample-vietnam_pdpd.yaml](compliance-samples/compliance-sample-vietnam_pdpd.yaml)               | Decreto 13/2023/NĐ-CP (vigência 1 jul 2024); enquadramento ministerial (segurança pública / cibercrime); padrões CCCD / CMND legado; **geolocalização precisa** (4 padrões); EN + vietnamita; nota de âmbito extraterritorial no YAML. |
| **Coreia do Sul (PIPA)**        | [compliance-sample-south_korea_pipa.yaml](compliance-samples/compliance-sample-south_korea_pipa.yaml)       | PIPA (개인정보 보호법), PIPC; RRN + registro comercial + passaporte com regex contextual; **geolocalização precisa** (4 padrões); 95+ termos ML (EN + coreano); transferência ao exterior, subcontratação, biometria, CCTV; nota setorial no YAML. |
| **UAE (PDPL)**                  | [compliance-sample-uae_pdpl.yaml](compliance-samples/compliance-sample-uae_pdpl.yaml)                       | Decreto-Lei 45/2021 + Regulamentos Gabinete 2023; UAE Data Office (UAEDO/TDRA); regex Emirates ID; **geolocalização precisa**; categorias sensíveis (saúde, biométrico, genético, racial, religioso, criminal, financeiro, dados de crianças); nota Free Zone (DIFC/ADGM são regimes separados); EN + AR. |
| **Egito (PDPL Lei 151/2020)**   | [compliance-sample-egypt_pdpl.yaml](compliance-samples/compliance-sample-egypt_pdpl.yaml)                   | Lei 151/2020 + Decreto 816/2025 (1 nov 2025); Centro de Proteção de Dados Pessoais (PDPC); rótulos contextuais de ID nacional (sem matcher nu de 14 dígitos); dados de crianças como sensíveis; sinal de permissão EDM; geolocalização precisa (JSON lat/lng + par CSV); notificação de violação ao Centro em 72 h; carência até **31 out 2026**; EN + AR. |
...
| **Etiópia (PDPP 1321/2024)**    | [compliance-sample-ethiopia_pdpp.yaml](compliance-samples/compliance-sample-ethiopia_pdpp.yaml)             | Proclamação 1321/2024 (vigência 24 jul 2024); ECA; rótulos contextuais de Fayda FIN (sem matcher nu de 12 dígitos); biométrico (impressão digital/rosto/íris); geolocalização precisa (JSON lat/lng + par CSV); notificação de violação em 72 h (revalidar); EN + rascunho em amárico (revisão nativa recomendada). |
| **Índia (DPDP)**                | [compliance-sample-india_dpdp.yaml](compliance-samples/compliance-sample-india_dpdp.yaml)                   | DPDP Act 2023, DPBI; regex Aadhaar/PAN; **geolocalização precisa** (mesma geometria VCDPA / EU GDPR); termos ML SDF / transferência internacional / dados de crianças; EN; regras em evolução — revalidar. |
| **Paquistão (PDPA Bill 2023)** | [compliance-sample-pakistan_pdpa.yaml](compliance-samples/compliance-sample-pakistan_pdpa.yaml)           | Personal Data Protection Bill 2023 (rascunho em 2026-08 — ainda não promulgado); NCPDP proposto (s.35); CNIC formatado + rótulos NADRA/NTN/passaporte; s.2(kk) sensível; s.14 crianças; geolocalização precisa (JSON lat/lng + par CSV); EN + urdu. |
| **Indonésia (Lei PDP)**        | [compliance-sample-indonesia_pdp.yaml](compliance-samples/compliance-sample-indonesia_pdp.yaml)             | Lei 27/2022 sobre proteção de dados pessoais; autoridade PDP; dado pessoal / específico; **geolocalização precisa** (4 padrões); EN + indonésio. |
| **China (PIPL)**                | [compliance-sample-china_pipl.yaml](compliance-samples/compliance-sample-china_pipl.yaml)                   | PIPL; CAC; regex contextual para ID de residente + código de crédito social unificado; **geolocalização precisa** (4 padrões); PI sensível, consentimento separado, transferência, IIC, menores; EN + chinês (UTF-8). |
| **Turquia (KVKK)**              | [compliance-sample-turkey_kvkk.yaml](compliance-samples/compliance-sample-turkey_kvkk.yaml)                 | Lei 6698 + Emenda 7499 (2024); Conselho KVKK; regex TC Kimlik No; **geolocalização precisa**; categorias especiais (raça, saúde, biométrico, genético, vida sexual, condenação criminal); VERBİS; notificação de violação em 72 h; EN + TR. |
| **Ucrânia (Lei PDP 2297-VI)**  | [compliance-sample-ukraine_pdp.yaml](compliance-samples/compliance-sample-ukraine_pdp.yaml)                 | Lei 2297-VI (2010, emendada 2023); convergência GDPR (candidatura UE 2022); IPN + passaporte biométrico e legado cirílico; **geolocalização precisa** (4 padrões); 98+ termos ML (EN + ucraniano); categorias sensíveis (Art. 7); notificação de violação em 72 h (Lei 3454-IX 2023); ROPA, DPIA, EPD; exceções de lei marcial; transferência transfronteiriça. |
| **Nova Zelândia (Privacy Act)** | [compliance-sample-new_zealand_privacy.yaml](compliance-samples/compliance-sample-new_zealand_privacy.yaml) | Privacy Act 2020; OPC; regex IRD/NHI/Passaporte; **geolocalização precisa**; 13 IPPs; regime de Violação Notificável; categorias sensíveis (saúde, biométrico, genético, etnia, orientação sexual, financeiro); salvaguardas IPP 12 para transferência internacional; EN. |
| **Rússia (152-FZ)**             | [compliance-sample-russia_152_fz.yaml](compliance-samples/compliance-sample-russia_152_fz.yaml)             | Lei federal 152-FZ (266-FZ 2022), Roskomnadzor; **geolocalização precisa**; regex SNILS e INN heurístico; categorias especiais (Art. 10); notificação obrigatória (72 h); localização (Art. 22.1); termos EN + RU; **revisar com frequência**. |
...
| **Israel (PPL + Emenda 13)**    | [compliance-sample-israel_ppl.yaml](compliance-samples/compliance-sample-israel_ppl.yaml)                   | PPL 5741-1981 + Emenda 13 (vigência 14 ago 2025); PPA; Controller/Processor; Highly Sensitive Information; rótulos contextuais de teudat zehut (sem matcher nu de 9 dígitos); geolocalização precisa (JSON lat/lng + par CSV); EN + HE. |
...
| **México (LFPDPPP)**            | [compliance-sample-mexico_lfpdppp.yaml](compliance-samples/compliance-sample-mexico_lfpdppp.yaml)             | LFPDPPP, INAI; datos personales / sensibles, ARCO, aviso de privacidad; CURP/RFC contextual; **geolocalização precisa** (4 padrões); ES + EN. |
...
| **Equador (LOPDP — 2021)**      | [compliance-sample-ecuador_lopdp.yaml](compliance-samples/compliance-sample-ecuador_lopdp.yaml)             | Ley Orgánica de Protección de Datos Personales; APDP; rótulos contextuais de cédula/RUC/passaporte; status migratório como dado sensível (Art. 4/25 — incomum na LatAm); geolocalização precisa (JSON lat/lng + par CSV); dados de crianças (Art. 25); notificação de violação Art. 43 (cinco dias); influenciada pelo GDPR; vigência plena maio 2023; ES + EN. |
| **Peru (Ley 29733 — 2011)**    | [compliance-sample-peru_ley29733.yaml](compliance-samples/compliance-sample-peru_ley29733.yaml)             | Ley de Protección de Datos Personales + DS 003-2013-JUS; ANPDP; rótulos contextuais DNI/RUC/passaporte/CE (sem matchers nus); Art. 2.5 sensível (saúde, ingresos económicos, convicções/sindicato); Art. 13.6 consentimento por escrito; Art. 17 dever de confidencialidade; geolocalização precisa (JSON lat/lng + par CSV); candidato à adesão à OCDE (roteiro 2022 — não é membro em 2026-08); ES + EN. |
...
| **EUA (CA AB 2273)**            | [compliance-sample-us_ca_ab2273_caadca.yaml](compliance-samples/compliance-sample-us_ca_ab2273_caadca.yaml) | Califórnia, código de desenho apropriado à idade — **rotulagem**; aplicabilidade depende de fatos e assessoria jurídica; regex de **geolocalização precisa** (mesma geometria VCDPA / RGPD UE) para inventário.            |
...
| **EUA (Virgínia — VCDPA)**      | [compliance-sample-us_va_vcdpa.yaml](compliance-samples/compliance-sample-us_va_vcdpa.yaml)                 | Virginia Consumer Data Protection Act: léxico sensível + regexes de **geolocalização precisa**; **sem** análise automática de isenções B2B/emprego — ver cabeçalho da amostra. |
| **EUA (Califórnia — CPRA/CCPA)** | [compliance-sample-us_ca_cpra.yaml](compliance-samples/compliance-sample-us_ca_cpra.yaml)              | CCPA alterada pela CPRA: exporta `CCPA_SSN` do core + CNH-CA / ITIN; regex de **geolocalização precisa** SPI (§ 1798.140(ae)(1)(H)); termos ML de privacidade do consumidor; enquadramento CPPA. |
...
## Frameworks de cibernética e governança de IA (evidência, não amostras) {#ciber-ia-governanca-evidencia-nao-amostras}

Conversas com compradores e auditores citam **NIS2**, **DORA** e o **AI Act da UE**. Esses instrumentos são sobretudo frameworks de **cibersegurança**, **resiliência operacional de TIC** ou **governança de IA** — não pacotes de vocabulário de PII/CUI. O Data Boar **apoia** via **evidência e inventário** (mapeamento de dados que pode alimentar registros de ativos, registros de TIC ou posicionamento de IA de baixo risco para um scanner determinístico, sem LLM). **Não** espere nem publique um `compliance-sample-*.yaml` para NIS2, DORA ou AI Act: isso seria **descompasso de categoria** e risco de overclaim. Para **CMMC**, o encaixe é outro: **descoberta de CUI é descoberta de dados** — veja [compliance-sample-us_cmmc_cui.yaml](compliance-samples/compliance-sample-us_cmmc_cui.yaml).

## Normas auditáveis e de gestão (papel de suporte) {#normas-auditaveis-e-de-gestao-papel-de-suporte}
...
**Categorias sensíveis estendidas (adjacência clínica, indicadores de PI, artefatos de segurança em dados armazenados):** O mesmo motor pode ser afinado para léxicos adicionais e formas de regex—muitas vezes com **consultoria** para delimitar alvos e reduzir falsos positivos. Posicionamento para compradores e DPOs (sem ligação a PMO interno): [COMPLIANCE_AND_LEGAL.pt_BR.md](COMPLIANCE_AND_LEGAL.pt_BR.md) (seção **Categorias sensíveis estendidas (configuração e serviços)**).
