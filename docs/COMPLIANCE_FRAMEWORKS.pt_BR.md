...
| **Equador (LOPDP — 2021)**      | [compliance-sample-ecuador_lopdp.yaml](compliance-samples/compliance-sample-ecuador_lopdp.yaml)             | Ley Orgánica de Protección de Datos Personales; APDP; rótulos contextuais de cédula/RUC/passaporte; status migratório como dado sensível (Art. 23 — incomum na LatAm); geolocalização precisa (JSON lat/lng + par CSV); dados de crianças (Art. 35); notificação de violação em 72 h; influenciada pelo GDPR; vigência plena maio 2023; ES + EN. |
