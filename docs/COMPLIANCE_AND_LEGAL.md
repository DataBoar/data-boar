...
- **Optional jurisdiction hints (DPO/counsel triage):** When enabled, the Excel **Report info** sheet can include **heuristic** notes when **finding metadata** (e.g. column/path/norm tags) suggests **possible** relevance to certain regimes (e.g. U.S. state consumer privacy framings, Japan APPI)—**not** a determination that a law applies, **not** a substitute for jurisdictional analysis, and **not** based on full cell dumps stored in SQLite. Operators opt in via config, CLI, API, or dashboard; see [USAGE.md](USAGE.md) and [ADR 0026](adr/ADR-0026-optional-jurisdiction-hints-dpo-facing-heuristic-metadata-only.md).
...
**U.S. — CMMC and Controlled Unclassified Information (technical inventory only):** Optional sample [compliance-sample-us_cmmc_cui.yaml](compliance-samples/compliance-sample-us_cmmc_cui.yaml) supports **discovery and mapping** of CUI markings, dissemination-control vocabulary, and export-control **triage signals** (ITAR/EAR) in configured sources. Data Boar **supports CMMC evidence / CUI discovery**; it does **not** certify CMMC, assess a CMMC level, perform an assessment, or determine ITAR/EAR applicability. Cyber/AI programme frameworks such as **NIS2**, **DORA**, and the **EU AI Act** are **evidence-supported**, not pattern-pack samples—see [COMPLIANCE_FRAMEWORKS.md](COMPLIANCE_FRAMEWORKS.md#cyber-ai-governance-evidence-not-samples).
...
- **ISO/IEC 38500** (**IT governance**): governing-body cycle to **evaluate**, **direct**, and **monitor** IT. Data Boar supplies **discovery evidence** for that cycle; it does **not** implement a board operating model. Catalogue: [ISO/IEC 38500](https://www.iso.org/standard/62816.html). Alignment tables: [ITSM_GOVERNANCE_ALIGNMENT.md](ITSM_GOVERNANCE_ALIGNMENT.md).
- **ISO/IEC 38505** (**governance of data**): extends the 38500 idea into the **data** domain (value of data, risks to treat, alignment of data management with governing intent). Catalogue: [ISO/IEC 38505](https://www.iso.org/standard/56639.html). The product **maps exposure** of sensitive data in configured systems; it does **not** set data strategy or certify 38505.
- **DMBOK** (DAMA International): a **body of knowledge** for data management and governance — **good practice**, **not** an ISO management-system standard. Starting point: [DAMA Body of Knowledge](https://www.dama.org/cpages/body-of-knowledge). Discovery output can feed security, metadata, and (partly) quality conversations; it is **not** a GDD platform.
...
**Internal roadmap:** optional sample profiles and deeper positioning may be tracked in the repository’s **plans** tree (maintainers: see **docs/README** — *Internal and reference*). **Positioning decision (architecture):** [ADR 0025 — Compliance positioning: evidence and inventory, not a legal-conclusion engine](adr/ADR-0025-compliance-positioning-evidence-inventory-not-legal-conclusion-engine.md).
