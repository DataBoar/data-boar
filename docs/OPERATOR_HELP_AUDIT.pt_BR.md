...
- **2026-08-28 (relatório ODS #553 Parte A):** allowlist `report.formats` `xlsx`/`ods`; `GET /report?format=ods` e `GET /reports/{session_id}?format=ods`. Superfícies: **`docs/USAGE*.md`**, **`docs/data_boar.5`**, **`api/templates/help.html`**, **`tests/operator_help_sync_manifest.py`**.
...
- **2026-08-28 (bind-gate #1714):** Bind **`--web`** fora do loopback **encerra com código 2** a menos que **`api.require_api_key: true`** e a chave esteja resolvida (chave ou segredo WebAuthn sem a flag não basta). Docs: **USAGE** (EN + pt-BR), **SECURITY** raiz, **`docs/ops/SECURE_DASHBOARD_AUTH_AND_HTTPS_HOWTO*`**. Testes: **`tests/test_host_resolution.py`**, **`tests/test_main_web_transport_cli.py`**.
