...
**Validação de criptografia forte / controles (`scan.validate_crypto`, `--validate-crypto`):** Flag opt-in (desligada por padrão). Quando ativa via config, CLI, caixa no dashboard ou corpo da API **`validate_crypto: true`**, o motor liga a validação de criptografia forte nessa execução (CLI / API / dashboard sobrescrevem o config). Quando off ou ausente, esse caminho é ignorado — sem mudança de comportamento. **Fase 2a–2d + 2.4:** conectores SQL, MongoDB, Redis, SMB, REST, Power BI e Dataverse honram opções de crypto no connect quando configuradas (`verify` / `verify_ssl` no httpx), fazem probe após conectar e gravam **`crypto_controls_audit`**; planilha Excel **Crypto & controls** (detalhes allowlisted — sem segredos, tokens ou URLs). Ver [USAGE.pt_BR.md](USAGE.pt_BR.md) e [USAGE.md](USAGE.md) (*Strong crypto / controls*).
