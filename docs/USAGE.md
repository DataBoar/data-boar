...
>
> **Compressed archives (this flag does not dispatch them):** `use_content_type` / **`--content-type-check`** does **not** choose archive format from magic. When **`scan_compressed`** is on, a share or filesystem file whose **extension** looks like a supported archive but whose **magic** disagrees (e.g. gzip bytes named `.tar.bz2`) is **not** expanded. The run records **`scan_failures`** with reason **`archive_type_mismatch`**. Expanding by magic instead (option (a) in [#1354](https://github.com/DataBoar/data-boar/issues/1354)) remains deferred.
>
