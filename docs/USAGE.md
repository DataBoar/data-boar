...
| `--l3-persist`          | `PATH`            | Explicitly persist L3 JSON (raw values) to `PATH`. Omitted = stdout only. Requires `--export-l3`. Not a default. POSIX: `0600`. Windows: owner DACL via `icacls /inheritance:r` (privileged SYSTEM/Administrators may remain). Containment not proven → exit **5**. |
