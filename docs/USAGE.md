...
**Phase 2a (SQL) + Phase 2c (Mongo/Redis):** After a successful connect, Data Boar probes TLS best-effort. **SQL:** PostgreSQL `pg_stat_ssl`, MySQL/MariaDB `Ssl_version` / `Ssl_cipher` when available; SQLite → `not_applicable`; otherwise config `sslmode` fallback. **MongoDB / Redis:** honor target `tls` / `ssl` (and `mongodb+srv://` / `rediss://` / allowlisted `tls=true` URI flags) on connect; probe live socket / client options when exposed; cert posture via allowlisted tokens only (`ssl_cert_reqs`, `tls_insecure` / `tlsAllowInvalidCertificates`). Results go to SQLite **`crypto_controls_audit`** and the Excel sheet **Crypto & controls** (`ok` / `warning` / `fail` / `not_available` / `not_applicable`, short details — **no** certificates, passwords, or connection strings). Criteria: prefer TLS ≥ 1.2; plaintext / `sslmode=disable` / TLS &lt; 1.2 → fail; TLS without cert verify → warning. Probe failures never fail the scan. REST/SMB probes and anonymisation inference are later phases.
