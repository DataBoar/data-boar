...
- **Respostas da API:** `POST /scan` (também `/start` e `/scan_database`) retorna **409** (`Audit already in progress.`) quando este processo já **reservou** o único slot de `AuditEngine` **no aceite da requisição**, não quando a thread em background começa depois. **429** é o teto separado de `rate_limit`. Endpoints de sessão/relatório retornam 404 com mensagem clara quando a sessão ou o relatório não existir. **HTTP 400** com **`Host`** inválido/não confiável é o **`TrustedHostMiddleware`** (`api.trusted_hosts` / `api.host`) — não é chave de API ausente; inclua o nome que os clientes realmente enviam e **reinicie** `--web`.
...
| ----------------------                      | --------------------------                                                                                                                                                                      | -----------------------------                                                                                         |
| **unreachable**                             | Rede do host/container de auditoria até o alvo: DNS, roteamento, firewall, VPN. Para Docker/Podman: veja [TROUBLESHOOTING_DOCKER_DEPLOYMENT.pt_BR.md](ops/TROUBLESHOOTING_DOCKER_DEPLOYMENT.pt_BR.md). | [Conectividade](TROUBLESHOOTING_CONNECTIVITY.pt_BR.md) · [Docker](ops/TROUBLESHOOTING_DOCKER_DEPLOYMENT.pt_BR.md)     |
| **auth_failed** / **authentication_failed** | Credenciais (usuário/senha, token, OAuth client_id/secret). Confira `*_from_env` ausentes (`--validate-config` WARN). Evite enviar a mesma credencial no header e no body. | [Credenciais e autenticação](TROUBLESHOOTING_CREDENTIALS_AND_AUTH.pt_BR.md)                                           |
...
| **redis_value_not_sampled**                  | O `TYPE` Redis não era string/hash/list/set/zset/stream (contagens no JSON **Details**). Não é falha de TCP. A varredura de nomes ainda rodou. Aumente cobertura convertendo esses tipos ou aceite a lacuna. | [USAGE.pt_BR.md](USAGE.pt_BR.md) (*Redis*) · [TECH_GUIDE.pt_BR.md](TECH_GUIDE.pt_BR.md) |
...
## Docker / Podman: conectar a dados remotos a partir do container

Muitas implantações usam a **imagem Docker** (ou a mesma imagem no **Podman**). O container precisa conseguir alcançar seus bancos de dados, shares de arquivos (NFS/SMB) e APIs.
...
- **CPU x86 anterior a 2011** onde `import numpy` morre com **`Illegal instruction`** (classe Core 2 / Celeron / Pentium — só `ssse3`, sem SSE4.2/POPCNT). O piso é **`x86-64-v1`**, não só “sem AVX”: wheels do PyPI miram **`x86-64-v2`** ou superior; variáveis de ambiente (`NPY_DISABLE_CPU_FEATURES`, `OPENBLAS_CORETYPE`) **não** ajudam porque o crash está na **baseline compilada**, não no despacho em runtime ([#929](https://github.com/DataBoar/data-boar/issues/929)). **`import core.engine` não morre mais neste caso:** o detector checa essas flags de CPU **antes** de importar numpy e segue no caminho regex de CPF/CNPJ (ML/DL pulados, aviso no log). **Para restaurar ML de verdade** nesta CPU, use **este wheelhouse** (caminho de produto **`[noavx]`**, não um extra do PyPI) — a instalação em dois passos `--no-index --find-links` abaixo. Numpy da distro (`apk add py3-numpy` / `apt install python3-numpy`) ou build de fonte com `-Dcpu-baseline=min` são fallbacks, não o stack verificado.
- Instalação **air-gapped** ou com egress restrito que precisa resolver offline.

**Release hospedado (verificado):** [wheelhouse-x86-64-v1-2026-07-29](https://github.com/DataBoar/data-boar-site/releases/tag/wheelhouse-x86-64-v1-2026-07-29) em `DataBoar/data-boar-site` — **54** assets `.whl`, `SHA256SUMS` anexo. Texto completo de instalação/verificação também no asset `README.md` do release.
...
#### Índice remoto (--extra-index-url)

O índice PEP 503 **`simple/`** do wheelhouse hospedado vive em **`DataBoar/data-boar-site`** ([PR #21](https://github.com/DataBoar/data-boar-site/pull/21); rastreio no produto [#1445](https://github.com/DataBoar/data-boar/issues/1445)). URL canônica:

`https://databoar.com.br/simple/`

(`https://databoar.github.io/data-boar-site/simple/` redireciona para o mesmo caminho no apex.)

```bash
# Resolve um pacote do wheelhouse pelo nome normalizado (exemplo: acelerador Rust)
pip install --extra-index-url https://databoar.com.br/simple/ boar-fast-filter

# Depois que o pipx instalou data-boar — inject a partir do índice remoto
pipx runpip data-boar install --extra-index-url https://databoar.com.br/simple/ boar-fast-filter
```

**Empate (tie-breaker):** `--extra-index-url` **mescla** candidatos com o PyPI; **não** prefere o wheelhouse. Quando os dois publicam a **mesma versão e a mesma platform tag** (ex.: numpy `2.5.1` manylinux/musllinux), o pip ainda pode instalar o wheel do **PyPI** (x86-64-v2+ / SIGILL em hosts min-spec). Para o stack de ML em musl ou **x86-64-v1**, mantenha o contrato de dois passos `--force-reinstall` / `--no-index` na subseção seguinte (pasta local ou wheels baixados). Versões locais PEP 440 (`+x86v1`) continuam no restante do #1182 — o índice sozinho **não** fecha essa lacuna.

Regenere a árvore HTML no repositório do **site** (`scripts/build-wheelhouse-index.py`) após cada release do wheelhouse. A árvore do produto **não** hospeda um builder duplicado.
...
- `--find-links` aceita **pasta local**, **URL direta de `.whl`** ou **página HTML de links** — **não** a página de **release** do GitHub. Com ~50 wheels, baixe para uma pasta primeiro (ou use o [índice remoto](#índice-remoto---extra-index-url) para resolver pelo nome e ainda force-reinstall o stack de ML abaixo).
...
| **Credenciais e autenticação** | API key no header vs body; Basic/Bearer/OAuth; `*_from_env` ausente; lockouts           | [TROUBLESHOOTING_CREDENTIALS_AND_AUTH.md](TROUBLESHOOTING_CREDENTIALS_AND_AUTH.md) | [TROUBLESHOOTING_CREDENTIALS_AND_AUTH.pt_BR.md](TROUBLESHOOTING_CREDENTIALS_AND_AUTH.pt_BR.md) |
| **Docker / Podman**            | Rodar em container; NFS/SMB; DB remoto; DNS; volumes; hostnames do Podman               | [TROUBLESHOOTING_DOCKER_DEPLOYMENT.md](ops/TROUBLESHOOTING_DOCKER_DEPLOYMENT.md)   | [TROUBLESHOOTING_DOCKER_DEPLOYMENT.pt_BR.md](ops/TROUBLESHOOTING_DOCKER_DEPLOYMENT.pt_BR.md)   |
