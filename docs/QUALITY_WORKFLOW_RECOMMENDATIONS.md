...
- **Implemented:** GitHub Actions workflow [**SBOM**](../.github/workflows/sbom.yml) — **CycloneDX JSON** from `uv export` + `cyclonedx-py`, then **`scripts/application_sbom.py`** merges **`Cargo.lock`** into the **same** application file (`sbom-python.cdx.json` / `sbom/sbom-application.cdx.json`); **Syft** on the built image (`sbom-docker-image.cdx.json` / `sbom/sbom-runtime.cdx.json`). See [SECURITY.md](../SECURITY.md), [RELEASE_INTEGRITY.md](RELEASE_INTEGRITY.md), [ADR 0003](adr/ADR-0003-sbom-roadmap-cyclonedx-then-syft.md). Local: [`scripts/generate-sbom.ps1`](../scripts/generate-sbom.ps1) / [`scripts/generate-sbom.sh`](../scripts/generate-sbom.sh). Check: [`scripts/check-sbom.sh`](../scripts/check-sbom.sh).
