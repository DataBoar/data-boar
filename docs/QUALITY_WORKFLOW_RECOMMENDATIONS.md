...
- **Local (optional):** `uvx semgrep@<tag> scan --config p/python --metrics=off` where **`<tag>`** is the `semgrep/semgrep:X.Y.Z` image tag in [`.github/workflows/semgrep.yml`](../.github/workflows/semgrep.yml) (read by `check-all-security-scans.*`; fail-closed if missing). Same **`--exclude-rule`** as the workflow (see **§ check-all unified mirror** below). Custom rules can live under `.semgrep/` later.
...
| **Opt-in `--enforced` / `-Enforced`** | Before security-sensitive PRs; when Semgrep CI is the worry | **+ Semgrep** — `uvx semgrep@X.Y.Z` where **X.Y.Z** is parsed from the [semgrep.yml](../.github/workflows/semgrep.yml) image tag (no second pin in the script; abort if unreadable), same **`--exclude-rule`**, **`--error .`**. Requires network for `uvx`. **Zizmor** local `uvx zizmor` is the PyPI CLI, not [zizmor.yml](../.github/workflows/zizmor.yml) `zizmor-action` — do not treat those as the same pin. |
...
| Workflow permission drift  | Fails in **Zizmor** job unless **`ZIZMOR_ENFORCE=false`**       | Surfaces before push                              |
...
- **Implemented:** GitHub Actions workflow [**SBOM**](../.github/workflows/sbom.yml) — **CycloneDX JSON** from `uv export` + `cyclonedx-py`, then **`scripts/application_sbom.py`** merges **`Cargo.lock`** into the **same** application file (`sbom-python.cdx.json` / `sbom/sbom-application.cdx.json`); **Syft** on the built image (`sbom-docker-image.cdx.json` / `sbom/sbom-runtime.cdx.json`). Local unsigned provenance record: [`scripts/emit-provenance.sh`](../scripts/emit-provenance.sh) (not SLSA; tag path still uses OIDC `data-boar.intoto.jsonl`). See [SECURITY.md](../SECURITY.md), [RELEASE_INTEGRITY.md](RELEASE_INTEGRITY.md), [ADR 0003](adr/ADR-0003-sbom-roadmap-cyclonedx-then-syft.md). Local: [`scripts/generate-sbom.ps1`](../scripts/generate-sbom.ps1) / [`scripts/generate-sbom.sh`](../scripts/generate-sbom.sh). Check: [`scripts/check-sbom.sh`](../scripts/check-sbom.sh).
