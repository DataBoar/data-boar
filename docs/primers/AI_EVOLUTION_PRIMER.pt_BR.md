...
| **Deep RL (DRL)** | **RL** com políticas ou redes de **valor profundas** (ex.: DQN, PPO em *pixels*) | Lida com estado **de alta dimensão**; mesmo ciclo orientado a recompensa do RL tabular | **Não** use **“RDL”** como sigla canônica—o usual em inglês é **DRL** (*deep reinforcement learning*). Herda riscos de **RL + DL** (*reward hacking*, falhas pouco transparentes) |
...
- **O mesmo encoder pequeno, dois papéis (não é LLM):** O DL opcional em [`core/dl_backend.py`](../../core/dl_backend.py) carrega **`sentence-transformers/all-MiniLM-L6-v2`** (padrão no código: 384 dimensões, classe ~80 MB) e treina um cabeçote **logístico** (`sklearn`) para classificar texto **sensível vs não sensível**. Isso é **embedding + classificação**, não geração de próximo token. **Latent Lynx** (satélite, **recuperação** local) usa a **mesma família MiniLM** como encoder de **similaridade / recuperação**, não como chatbot. **Nenhum** dos dois caminhos é **LLM**; **nenhum** é **generativo**. Este item é **vocabulário** (ver [GLOSSARY.pt_BR.md](../GLOSSARY.pt_BR.md) **modelo de linguagem (LM)** no §8b vs **LLM** no §8) — não um *benchmark* nem afirmação de acurácia.
