...
- **Same small encoder, two roles (not an LLM):** Optional DL in [`core/dl_backend.py`](../../core/dl_backend.py) loads **`sentence-transformers/all-MiniLM-L6-v2`** (in-code default: 384-dim, ~80 MB-class) and trains a **logistic** head (`sklearn`) to classify **sensitive vs non-sensitive** text. That is **embedding + classification**, not next-token generation. **Latent Lynx** (satellite, local **retrieval**) uses the **same MiniLM family** as an encoder for **similarity / retrieval**, not as a chatbot. **Neither** path is an **LLM**; **neither** is **generative**. This bullet is **vocabulary** (see [GLOSSARY.md](../GLOSSARY.md) **language model (LM)** in §8b vs **LLM** in §8)—not a benchmark or accuracy claim.
