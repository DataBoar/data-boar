...
```
  ...
  **`native_packages[]`** (optional, [#1408](https://github.com/DataBoar/data-boar/issues/1408)): hashes of the nfpm artifacts on the same Release. Licensing `--check` still verifies **`files[]` only**. Air-gap steps: [ops/NATIVE_PACKAGE_RELEASE.md](ops/NATIVE_PACKAGE_RELEASE.md).
...
**CycloneDX JSON** artifacts for the Python dependency tree and for the **Docker image** built from this repository are generated by the GitHub Actions workflow [**SBOM**](../.github/workflows/sbom.yml), with the same filenames documented in [**SECURITY.md**](../SECURITY.md) (**`sbom-python.cdx.json`**, **`sbom-docker-image.cdx.json`**). From the **next** `v*` GitHub tag, that job also attaches Sigstore SLSA provenance (**`data-boar.intoto.jsonl`**) via OIDC (`actions/attest-build-provenance`) — no Cosign private key; older Releases stay unsigned. Decision record: [**ADR 0003**](adr/ADR-0003-sbom-roadmap-cyclonedx-then-syft.md). Local regeneration (Docker required for the image SBOM): [**scripts/generate-sbom.ps1**](../scripts/generate-sbom.ps1).
