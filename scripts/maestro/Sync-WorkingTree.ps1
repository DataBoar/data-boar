...
.SYNOPSIS
 ...
 Missão principal: Sub-Orquestrador de sincronismo em pré-flight para o teste Completão funcionar no Lab-Op a partir do repo no gh ou da working tree no ambiente canônico do primary Windows dev workstation (L-series).
#>
...
if ($Ref -eq "WorkingTree") {
    ...
    # Comando Rsync.
    # Nice catch excluding docs/private from rsyncCmd so it will not exfiltrate the dev workstation tree. Thanks Gemini.
