...
<#
.SYNOPSIS
      Data Boar Orchestrator - Hybrid Lab-Op High-Density (v173)
...
function Get-HybridNodesFromManifest {
    ...
    $roleDefs = @(
        ...
        @{ Name = "WORKSTATION"; Regex = '(?i)WORKSTATION|lab-node-01'; Type = "podman" },
...
function Deploy-ConfigTrack {
    ...
    Set-Content -LiteralPath $tempPath -Value $content -Encoding ascii
    & scp.exe -q $tempPath "$($Node.SshHost):${remoteDir}/config_databoar.yaml"
