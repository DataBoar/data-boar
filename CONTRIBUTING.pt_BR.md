...
- **CI:** O GitHub Actions executa testes e **auditoria de dependências** (`uv run pip-audit`) em todo push/PR para `main` (ou `master`). A matriz Ubuntu padrão instala só **`--extra shares`**; o job **`test-extras`** (Python 3.13) instala extras SQL **exceto** **`mariadb`**, mais **`nosql` / `compressed` / `dataformats`**, para os testes de conectores opcionais não pularem em silêncio (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md)) *CI*). Os testes gateados pelo clone do Maestro são deselecionados nesse job de extras (CI público / forks; spinout maestro#8). O extra `mariadb` fica de fora no 3.13 até um conector estável no PyPI importar (1.1.14 `SyntaxError`; 2.0.0 ainda rc). A célula Test em **Python 3.13** também publica **`coverage.xml`** (pytest-cov) para o Sonar; as outras células da matriz não. O job Test **retenta** o download do tarball do gitleaks (o checksum continua obrigatório) e **faz fallback** para `pip install uv==0.11.2` se o `setup-uv` falhar (#1842). Os PRs devem resolver qualquer falha de auditoria (corrigir ou atualizar dependências vulneráveis) antes do merge. Quando o SonarQube/SonarCloud estiver habilitado (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md))), trate os problemas reportados para que o quality gate permaneça verde.
