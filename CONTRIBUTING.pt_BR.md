...
pip install 'data-boar[mssql]'       # SQL Server (pymssql; driver padrão: mssql)
...
1. **Instale as dependências com uv (recomendado)**
   ...
   **PowerShell (`pwsh`) — ativação manual do venv:** quando existir `.\.venv\Scripts\Activate.ps1`, execute-o na raiz. **Não** invoque o shim `activate` **sem extensão** dentro de `.venv\Scripts\` pelo caminho completo — o PowerShell só trata **`Activate.ps1`** como script (rodar `…\Scripts\activate` sem `.ps1` dá *não reconhecido*). Se o `.ps1` faltar após **`uv sync`**, use **`uv run …`** ou recrie o ambiente (apague `.venv` e rode **`uv sync`** de novo). No dia a dia prefira **`uv run …`**.
...
- **Commits assinados** são obrigatórios (proteção clássica e ruleset `restriction baseline`). O Dependabot não recebe push unsigned do Actions — use o handoff do workflow de sync ou um PR substituto assinado ([#1419](https://github.com/DataBoar/data-boar/issues/1419); [docs/ops/DEPENDABOT_REQUIREMENTS_SYNC.pt_BR.md](docs/ops/DEPENDABOT_REQUIREMENTS_SYNC.pt_BR.md)).
...
- **Lint, Bandit, pytest Windows, Semgrep, CodeQL, Sonar, Zizmor** costumam rodar no PR. Trate um job vermelho como pare mesmo quando o GitHub ainda permitiria merge. O **Zizmor** roda em **todo** push/PR (**sem** filtro de path) e **falha** salvo **`ZIZMOR_ENFORCE=false`**; esse job **não** é check obrigatório. O **CodeQL** usa `.github/workflows/codeql.yml`; **não** deixe o **default setup** do GitHub ligado ao mesmo tempo ([docs/TESTING.pt_BR.md](docs/TESTING.pt_BR.md) *CodeQL*).
...
- **CI:** O GitHub Actions executa testes e **auditoria de dependências** (`uv run pip-audit`) em todo push/PR para `main` (ou `master`). A matriz Ubuntu padrão instala só **`--extra shares`**; o job **`test-extras`** (Python 3.13) instala extras SQL **exceto** **`mariadb`**, mais **`nosql` / `compressed` / `dataformats`**, para os testes de conectores opcionais não pularem em silêncio (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md)) *CI*). Os testes gateados pelo clone do Maestro são deselecionados nesse job de extras (CI público / forks; spinout maestro#8). O extra `mariadb` fica de fora no 3.13 até um conector estável no PyPI importar (1.1.14 `SyntaxError`; 2.0.0 ainda rc). A célula Test em **Python 3.13** também publica **`coverage.xml`** (pytest-cov) para o Sonar; as outras células da matriz não. O job Test **retenta** o download do tarball do gitleaks e depois faz **`sha256sum` do binário linux_x64 extraído** (`scripts/tool-pins.sh`, #1933 — não o `checksums.txt`) e **faz fallback** para `pip install uv==0.11.2` se o `setup-uv` falhar (#1842). Scan estrito de segredos + OSV: [docs/TESTING.pt_BR.md](docs/TESTING.pt_BR.md) *Gitleaks + OSV*. Os PRs devem resolver qualquer falha de auditoria (corrigir ou atualizar dependências vulneráveis) antes do merge. Quando o SonarQube/SonarCloud estiver habilitado (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md))), trate os problemas reportados para que o quality gate permaneça verde.
- **Dependências:** A fonte de verdade das bibliotecas é o **`pyproject.toml`** (toolchain uv). O arquivo **`uv.lock`** fixa a árvore exata de dependências resolvidas para que as instalações sejam reproduzíveis e os usuários fiquem protegidos de quebras acidentais quando uma dependência atualiza (“funcionou ontem”). Declare todas as dependências em **`pyproject.toml`** (prefira **versões mínimas `>=`**; use pin `==` só quando necessário). Ao adicionar ou alterar deps, execute `uv lock`, depois `uv export --frozen --no-emit-project -o requirements.txt`, e faça commit de **pyproject.toml**, **uv.lock** e **requirements.txt**. Não edite `uv.lock` nem `requirements.txt` à mão para mudanças de versão. O CI executa `uv sync` (que usa o `uv.lock`) e em seguida `pip-audit`, ou seja, o ambiente bloqueado é o que é testado e auditado. O teste **`tests/test_dependency_artifacts_sync.py`** falha se o **`uv.lock`** estiver defasado em relação ao **`pyproject.toml`** (`uv lock --check`) ou se o **`requirements.txt`** não for exatamente a saída do **`uv export`** desse lock — isso pega alterações acidentais em um único arquivo (por exemplo PRs do Dependabot que só mexem em **`requirements.txt`**).
- **Dependabot / automação:** O Dependabot abre PRs semanais para pip e GitHub Actions. Ao aplicar uma atualização de dependência (ex.: de um PR do Dependabot), atualize primeiro o **`pyproject.toml`** (suba a versão mínima), execute `uv lock` e `uv export --frozen --no-emit-project -o requirements.txt`, e faça commit dos três arquivos. Faça merge dos PRs de dependência somente após o CI (testes e auditoria) passar. O Dependabot ajuda a sinalizar quando atualizar; agir nesses PRs (ou antes de uma release estável) mantém o lockfile atualizado, compatível e seguro. Para PRs **de segurança** do Dependabot, usamos o SLA opcional em [SECURITY.md](SECURITY.md) ([pt-BR](SECURITY.pt_BR.md)) (Resposta de segurança).
...
1. Siga o **[ADR 0005](docs/adr/ADR-0005-ci-github-actions-supply-chain-pins.md)** — fixe actions de terceiros em **SHA de commit completo (40 caracteres)** (mantenha a tag legível em **comentário** YAML na mesma linha); fixe o CLI **uv** com **`version:`** semver explícito em **`ci.yml`** — **não** `"latest"`.
1. Rode **`uv run pytest tests/test_github_workflows.py -v`** (módulo completo: pins SHA, padrão de passo de webhook Slack, forma de Semgrep/Gitleaks/SBOM, lint do **`ci.yml`** = **`pre-commit run --all-files`**, política do arquivo Slack arquivado) antes do push.
