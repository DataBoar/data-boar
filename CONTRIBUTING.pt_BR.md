...
- **CI:** O GitHub Actions executa testes e **auditoria de dependências** (`uv run pip-audit`) em todo push/PR para `main` (ou `master`). A matriz Ubuntu padrão instala só **`--extra shares`**; o job **`test-extras`** (Python 3.13) instala extras SQL **exceto** **`mariadb`**, mais **`nosql` / `compressed` / `dataformats`**, e faz checkout do **DataBoar/maestro** (`MAESTRO_ROOT`, secret **`MAESTRO_CHECKOUT_TOKEN`**) para os testes de conectores opcionais e os guardrails consumer-side do Maestro não pularem em silêncio (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md)) *CI*). O extra `mariadb` fica de fora no 3.13 até um conector estável no PyPI importar (1.1.14 `SyntaxError`; 2.0.0 ainda rc). Os PRs devem resolver qualquer falha de auditoria (corrigir ou atualizar dependências vulneráveis) antes do merge. Quando o SonarQube/SonarCloud estiver habilitado (veja [docs/TESTING.md](docs/TESTING.md) ([pt-BR](docs/TESTING.pt_BR.md))), trate os problemas reportados para que o quality gate permaneça verde.
