...
def test_required_extra_for_bare_mssql_defaults_to_mssql_extra() -> None:
    assert required_extra_for_driver("mssql") == "mssql"


def test_required_extra_for_explicit_mssql_pyodbc() -> None:
    assert required_extra_for_driver("mssql+pyodbc") == "mssql-pyodbc"
...
def test_ensure_sql_driver_missing_mssql(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "connectors.sql_driver_deps._module_available",
        lambda _name: False,
    )
    with pytest.raises(ImportError, match=r"data-boar\[mssql\]"):
        ensure_sql_driver_available("mssql")


def test_ensure_sql_driver_missing_mssql_pyodbc(
    ...
    with pytest.raises(ImportError, match=r"data-boar\[mssql-pyodbc\]"):
        ensure_sql_driver_available("mssql+pyodbc")
