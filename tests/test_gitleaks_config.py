"""Gitleaks kombi — ``gitleaks detect --config security/gitleaks.toml`` (PLAN_NO_COAUTHORSHIP_GATE).
...
def test_gitleaks_toml_extend_and_no_coauthorship_rule() -> None:
    """Maintainer policy must extend defaults and declare the no-coauthorship governance rule."""
...
def test_gitleaks_detect_uses_repo_config() -> None:
    """Run ``gitleaks detect --config security/gitleaks.toml`` from repo root (kombi contract)."""
    proc = subprocess.run(
    ...
    assert proc.returncode == 0, (
        "gitleaks detect --config security/gitleaks.toml failed\n"
        f"stdout:\n{proc.stdout}\nstderr:\n{proc.stderr}"
    )
