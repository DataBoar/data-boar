...
2. From the **product repo root**, run **`scripts/private-git-sync.ps1`**; add **`-Push`** when the operator’s policy or request includes **all non-public mirrors** (configured **`lab-*`** remotes on the canonical host list **mini-bt, latitude, t14, alpine-emachines** — **not pi3b** — **and**, on Windows, push to a **bare `notes-sync.git` on the VeraCrypt-mounted drive** when present — the script probes **`Z:`** first (operator default), then **`Y:`** as fallback). On **Linux/macOS**, use **`scripts/private-git-sync.sh --push`** (pCloud at **`~/pCloudDrive/lab-private-backup/notes-sync/`** when mounted). **`-Push`** / **`--push`** also copies homelab secret env leaf files (e.g. **`.env.bitwarden.local`**) into VeraCrypt **`working/homelab/`** and **excludes** them from the **pCloud** mirror. When the operator asks to **align**, **verify drift**, or **sync** private history, **do not** ask redundant permission to include obvious backup targets — see **`operator-evidence-backup-no-rhetorical-asks.mdc`** and **[ADR 0040](../../../docs/adr/ADR-0040-assistant-private-stack-evidence-mirrors-default.md)**.
