...
- **PyPI distribution name:** `pyproject.toml` declares **`name = "data-boar"`**. The project **is published** on the Python Package Index ([pypi.org/project/data-boar](https://pypi.org/project/data-boar/)):
  ...
  Pre-release lines on **`main`** (for example **`1.8.0-rc`**) stay **git-only** — they are not the PyPI wheel. For contributors, or to run that working tree, use a **git clone** with **`uv sync`** or **`pip install -e .`**. The tree still contains the historical **`lgpd_crawler`** Python package for imports and continuity — that is implementation layout, not the product name. Rename context: **[ADR 0014](docs/adr/ADR-0014-rename-repo-and-package-python3-lgpd-crawler-to-data-boar.md)**.
...
1. **Run the full local gate (recommended before a PR)** — one command:

   - **Windows:** `.\scripts\check-all.ps1` from the repo root.
   - **Linux / macOS:** `./scripts/check-all.sh` (make executable: `chmod +x scripts/check-all.sh`). Requires **`uv`** on `PATH`; the gate runs **`uv run python scripts/gatekeeper_audit.py`** for staged PII seeds (no **`pwsh`** dependency — **`#560`**).
...
- **Signed commits** are required (classic protection and ruleset `restriction baseline`). Dependabot cannot receive unsigned pushes from Actions — use the sync workflow handoff or a signed supersede PR ([#1419](https://github.com/DataBoar/data-boar/issues/1419); [docs/ops/DEPENDABOT_REQUIREMENTS_SYNC.md](docs/ops/DEPENDABOT_REQUIREMENTS_SYNC.md)).
...
- **Lint, Bandit, Windows pytest, Semgrep, CodeQL, Sonar, Zizmor** usually run on the PR. Treat a red job as a stop even when GitHub would still allow merge. **Zizmor** runs on **every** push/PR (**no** path filter) and **fails** unless **`ZIZMOR_ENFORCE=false`**; that job is **not** a required check. **CodeQL** uses `.github/workflows/codeql.yml`; do not leave GitHub **default setup** enabled at the same time ([docs/TESTING.md](docs/TESTING.md) *CodeQL*).
...
- **CI:** GitHub Actions run tests and **dependency audit** (`uv run pip-audit`) on every push/PR to `main` (or `master`). The default Ubuntu matrix installs **`--extra shares` only**; job **`test-extras`** (Python 3.13) installs SQL extras **except** **`mariadb`**, plus **`nosql` / `compressed` / `dataformats`**, so optional-connector tests are not silent skips (see [docs/TESTING.md](docs/TESTING.md) *CI*). Maestro clone-gated tests are deselected from that extras job (public CI / forks; spinout maestro#8). The `mariadb` extra is omitted on 3.13 until a stable PyPI connector imports (1.1.14 `SyntaxError`; 2.0.0 still rc). The **Python 3.13** Test cell also publishes **`coverage.xml`** (pytest-cov) for Sonar; other matrix cells do not. The Test job **retries** the gitleaks tarball download then **`sha256sum`s the extracted linux_x64 binary** (`scripts/tool-pins.sh`, #1933 — not `checksums.txt`) and **falls back** to `pip install uv==0.11.2` if `setup-uv` fails (#1842). Strict secret scan + OSV: [docs/TESTING.md](docs/TESTING.md) *Gitleaks + OSV*. PRs must resolve any audit failures (fix or upgrade vulnerable dependencies) before merge. When SonarQube/SonarCloud is enabled (see [docs/TESTING.md](docs/TESTING.md)), address reported issues so the quality gate stays green.
- **Dependencies:** The source of truth for libraries is **`pyproject.toml`** (uv toolchain). The **`uv.lock`** file pins the exact resolved dependency tree so installs are reproducible and users are protected from “it worked yesterday” breakage when a dependency updates. Declare all runtime and dev dependencies in **`pyproject.toml`** (prefer **minimum versions `>=`**; pin `==` only when needed). When you add or change deps, run `uv lock`, then `uv export --frozen --no-emit-project -o requirements.txt`, and commit **pyproject.toml**, **uv.lock**, and **requirements.txt**. Do not edit `uv.lock` or `requirements.txt` by hand for version changes. CI runs `uv sync` (which uses `uv.lock`) and then `pip-audit`, so the locked environment is what is tested and audited. **`tests/test_dependency_artifacts_sync.py`** fails if `uv.lock` is stale versus **`pyproject.toml`** (`uv lock --check`) or if **`requirements.txt`** is not exactly the output of **`uv export`** from that lock—catching accidental single-file bumps (for example Dependabot PRs that only touch **`requirements.txt`**).
- **Dependabot / automation:** Dependabot opens weekly PRs for pip and GitHub Actions. When applying a dependency update (e.g. from a Dependabot PR), update **`pyproject.toml`** first (bump the minimum version), then run `uv lock` and `uv export --frozen --no-emit-project -o requirements.txt`, and commit all three files. Merge dependency PRs only after CI (tests and audit) pass. Dependabot helps signal when to refresh dependencies; acting on those PRs (or before a stable release) keeps the lockfile updated, compatible, and safe. For **security** Dependabot PRs, we use the optional SLA in [SECURITY.md](SECURITY.md) (Security response).
...
1. Follow **[ADR 0005](docs/adr/ADR-0005-ci-github-actions-supply-chain-pins.md)** — pin third-party actions to a **full 40-character commit SHA** (keep the human tag in a YAML **comment** on the same line); pin the **uv** CLI with an explicit **`version:`** semver in **`ci.yml`** — **not** `"latest"`.
...
- **Lockfile and audit:** Run `uv lock` to refresh the lockfile from current `pyproject.toml`, then `uv sync` and `uv run pip-audit`. Fix or upgrade any high/critical findings, then run `uv export --frozen --no-emit-project -o requirements.txt` and commit **uv.lock** and **requirements.txt** so the release is reproducible, compatible, and safe. This protects users from accidental breakage while keeping the release on audited dependencies.
...
- **[docs/ops/BRANCH_PROTECTION.md](docs/ops/BRANCH_PROTECTION.md)** ([pt-BR](docs/ops/BRANCH_PROTECTION.pt_BR.md)) — What GitHub requires on `main`.
