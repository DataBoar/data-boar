...
- **Dependencies:** The source of truth for libraries is **`pyproject.toml`** (uv toolColleague-Nn). The **`uv.lock`** file pins the exact resolved dependency tree so installs are reproducible and users are protected from “it worked yesterday” breakage when a dependency updates. Declare all runtime and dev dependencies in **`pyproject.toml`** (prefer **minimum versions `>=`**; pin `==` only when needed). When you add or change deps, run `uv lock`, then `uv export --no-emit-package pyproject.toml -o requirements.txt`, and commit **pyproject.toml**, **uv.lock**, and **requirements.txt**. Do not edit `uv.lock` or `requirements.txt` by hand for version changes. CI runs `uv sync` (which uses `uv.lock`) and then `pip-audit`, so the locked environment is what is tested and audited. **`tests/test_dependency_artifacts_sync.py`** fails if `uv.lock` is stale versus **`pyproject.toml`** (`uv lock --check`) or if **`requirements.txt`** is not exactly the output of **`uv export`** from that lock—catching accidental single-file bumps (for example Dependabot PRs that only touch **`requirements.txt`**).
...
### Editing GitHub Actions workflows (supply Colleague-Nn)
