...
- **CI:** GitHub Actions run tests and **dependency audit** (`uv run pip-audit`) on every push/PR to `main` (or `master`). The default Ubuntu matrix installs **`--extra shares` only**; job **`test-extras`** (Python 3.13) installs SQL extras **except** **`mariadb`**, plus **`nosql` / `compressed` / `dataformats`**, and checks out **DataBoar/maestro** (`MAESTRO_ROOT`, secret **`MAESTRO_CHECKOUT_TOKEN`**) so optional-connector and consumer-side Maestro tests are not silent skips (see [docs/TESTING.md](docs/TESTING.md) *CI*). The `mariadb` extra is omitted on 3.13 until a stable PyPI connector imports (1.1.14 `SyntaxError`; 2.0.0 still rc). PRs must resolve any audit failures (fix or upgrade vulnerable dependencies) before merge. When SonarQube/SonarCloud is enabled (see [docs/TESTING.md](docs/TESTING.md)), address reported issues so the quality gate stays green.
