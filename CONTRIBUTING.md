...
- **CI:** GitHub Actions run tests and **dependency audit** (`uv run pip-audit`) on every push/PR to `main` (or `master`). The default Ubuntu matrix installs **`--extra shares` only**; job **`test-extras`** (Python 3.13) installs SQL extras **except** **`mariadb`**, plus **`nosql` / `compressed` / `dataformats`**, so optional-connector tests are not silent skips (see [docs/TESTING.md](docs/TESTING.md) *CI*). Maestro clone-gated tests are deselected from that extras job (public CI / forks; spinout maestro#8). The `mariadb` extra is omitted on 3.13 until a stable PyPI connector imports (1.1.14 `SyntaxError`; 2.0.0 still rc). The **Python 3.13** Test cell also publishes **`coverage.xml`** (pytest-cov) for Sonar; other matrix cells do not. The Test job **retries** the gitleaks tarball download (checksum still required) and **falls back** to `pip install uv==0.11.2` if `setup-uv` fails (#1842). PRs must resolve any audit failures (fix or upgrade vulnerable dependencies) before merge. When SonarQube/SonarCloud is enabled (see [docs/TESTING.md](docs/TESTING.md)), address reported issues so the quality gate stays green.
...
- **Secrets and logging:** Confirm no API keys or passwords in logs; config file and env handling restrict access to trusted users (see [SECURITY.md](SECURITY.md)). Ensure **config file permissions** restrict read/write to trusted users and that **config.yaml** (and any file with secrets) is **not committed** (see .gitignore). When adding or changing log statements, do not log raw config, request bodies, or credentials; persist or print failure/exception text via `core.validation.sanitize_log_text` or `core.validation.clean_error` (not raw `str(e)`), so SQL/HTTP client bodies cannot leak into SQLite or logs (regression: `test_redact_*`, `test_sanitize_log_text_*`, `test_clean_error_*`, `test_save_failure_persists_*` in `tests/test_security.py` / `tests/test_database.py`).
